...
2. 不支持变量的动态设置。
3. 不支持变量表达式的计算。
4. 不支持suspend，disconnect功能。
5. 不支持条件断点。
6. 不支持表达式计算。
7. 不支持运行到下一个异常处。
8. 不支持在debug configuration页面修改debug对象。 
9. 调试函数或存储过程支持参数类型：
    - 数值型: tinyint， smallint，int，bigint，float，double，number 
    - 字符型： varchar，char，nchar，nvarchar
    - bool型：boolean
...
使用PL调试功能需要的权限依照所调试的存储过程中对不同对象的操作而改变。
