...
class MySQLDatabaseExportWizard extends AbstractToolWizard<MySQLCatalog> implements IExportWizard {
    ...
    public MySQLDatabaseExportWizard(MySQLCatalog catalog) {
        ...
        this.outputFile = new File(UIUtils.getCurDialogFolder(), catalog.getName() + "-" + RuntimeUtils.getCurrentTimeStamp() + ".sql"); //$NON-NLS-1$ //$NON-NLS-2$
	}
    ...
    {
        UIUtils.setCurDialogFolder(outputFile.getParentFile().getAbsolutePath());
