...
/**
 * GZIP/ZLIB stream value manager
 */
public class GzipStreamValueManager implements IStreamValueManager {
    ...
    public MatchType matchesTo(@NotNull DBRProgressMonitor monitor, @NotNull DBSTypedObject attribute, @Nullable DBDContent value) {
                ...
                if (storage != null) {
                    try (InputStream stream = storage.getContentStream()) {
                        if (stream != null) {
                            ...
                            if (GzipPanelEditor.isGzipHeader(header) || GzipPanelEditor.isZlibHeader(header)) {
        ...
        return ContentUtils.isTextContent(value) ? MatchType.NONE : MatchType.APPLIES;
    }

    @Override
    public IStreamValueEditor createPanelEditor(@NotNull final IValueController controller) throws DBException {
        return new GzipPanelEditor();
    }
