 ...
 */
public class CubridExecutionContext extends JDBCExecutionContext implements DBCExecutionContextDefaults<CubridCatalog, CubridSchema> {
    private static final Log log = Log.getLog(CubridExecutionContext.class);

    private String selectedEntityName;

    public CubridExecutionContext(@NotNull JDBCRemoteInstance instance, String purpose) {
    ...
    void determineSelectedEntity(DBRProgressMonitor monitor) {
        CubridDataSource dataSource = this.getDataSource();
            ...
            if (CommonUtils.isEmpty(dataSource.getQueryGetActiveDB())) {
                {
                    try {
                        ...
                        if (dataSource.getSelectedEntityType() == null && !CommonUtils.isEmpty(selectedEntityName)) {
                            dataSource.setSelectedEntityType(CubridConstants.ENTITY_TYPE_CATALOG);
                ...
                if (CommonUtils.isEmpty(selectedEntityName)) {
                    ...
                    try {
                        ...
                        if (dataSource.getSelectedEntityType() == null && !CommonUtils.isEmpty(selectedEntityName)) {
                            dataSource.setSelectedEntityType(CubridConstants.ENTITY_TYPE_SCHEMA);
                            dataSource.setSelectedEntityFromAPI(true);
                        }
                    } catch (Throwable e) {
                        log.debug(e);
    ...
    void initDefaultsFrom(DBRProgressMonitor monitor, CubridExecutionContext context) throws DBCException {
        CubridCatalog defaultCatalog = context.getDefaultCatalog();
        ...
        if (defaultCatalog != null && context.supportsCatalogChange()) {
            ...
            CubridSchema defaultSchema = context.getDefaultSchema();
            if (defaultSchema != null) {
                entityName = defaultSchema.getName();
            }
        }
        if (entityName != null) {
            CubridDataSource dataSource = getDataSource();
    ...
    @Override
    public CubridCatalog getDefaultCatalog() {
        if (!CommonUtils.isEmpty(selectedEntityName)) {
            CubridDataSource dataSource = getDataSource();
            if (dataSource.hasCatalogs()) {
                if (dataSource.getSelectedEntityType() == null || dataSource.getSelectedEntityType().equals(CubridConstants.ENTITY_TYPE_CATALOG) || !dataSource.hasSchemas()) {
    ...
    @Override
    public CubridSchema getDefaultSchema() {
        if (!CommonUtils.isEmpty(selectedEntityName)) {
            CubridDataSource dataSource = getDataSource();
            if (!dataSource.hasCatalogs() && dataSource.hasSchemas()) {
                if (dataSource.getSelectedEntityType() == null || dataSource.getSelectedEntityType().equals(CubridConstants.ENTITY_TYPE_SCHEMA) || !dataSource.hasCatalogs()) {
    ...
    public boolean supportsCatalogChange() {
        CubridDataSource dataSource = getDataSource();
        if (!(dataSource.getInfo() instanceof CubridDataSourceInfo)) {
            return true;
        }
        final CubridDataSourceInfo info = (CubridDataSourceInfo) dataSource.getInfo();
        if (dataSource.isSelectedEntityFromAPI() || !CommonUtils.isEmpty(dataSource.getQuerySetActiveDB())) {
            ...
            if (dataSource.hasCatalogs()) {
                return (CubridConstants.ENTITY_TYPE_CATALOG.equals(dataSource.getSelectedEntityType()) || !dataSource.hasSchemas()) && info.supportsCatalogSelection();
            }
        }
        return false;
    }
    ...
    public boolean supportsSchemaChange() {
        ...
        if (dataSource.isSelectedEntityFromAPI() || !CommonUtils.isEmpty(dataSource.getQuerySetActiveDB())) {
            ...
            if (dataSource.hasSchemas()) {
                return (CubridConstants.ENTITY_TYPE_SCHEMA.equals(dataSource.getSelectedEntityType()) || !dataSource.hasCatalogs()) && info.supportsSchemaSelection();
            }
        }
        return false;
    }
    ...
    @Override
    public void setDefaultCatalog(DBRProgressMonitor monitor, CubridCatalog catalog, CubridSchema schema) throws DBCException {
        if (catalog == null) {
            log.debug("Null current catalog");
            return;
        }
        CubridDataSource dataSource = getDataSource();
        CubridCatalog oldSelectedCatalog = getDefaultCatalog();
        DBCTransactionManager txnManager = null;
        boolean autoCommit = true;
        boolean needToSetAutocommit = false;
        try (JDBCSession session = openSession(monitor, DBCExecutionPurpose.UTIL, "Set active catalog")) {
    ...
    @Override
    public void setDefaultSchema(DBRProgressMonitor monitor, CubridSchema schema) throws DBCException {
        if (schema == null) {
            log.debug("Null current schema");
            return;

        }
        CubridSchema oldSelectedSchema = getDefaultSchema();
    ...
    public CubridObjectContainer getDefaultObject() {
        if (!CommonUtils.isEmpty(selectedEntityName)) {
            CubridDataSource dataSource = getDataSource();
            if (dataSource.hasCatalogs()) {
                if (dataSource.getSelectedEntityType() == null || dataSource.getSelectedEntityType().equals(CubridConstants.ENTITY_TYPE_CATALOG)) {
                ...
                /*if (selectedEntityType == null || selectedEntityType.equals(CubridConstants.ENTITY_TYPE_SCHEMA)) */{
                    return dataSource.getSchema(selectedEntityName);
                }
            }
        }
