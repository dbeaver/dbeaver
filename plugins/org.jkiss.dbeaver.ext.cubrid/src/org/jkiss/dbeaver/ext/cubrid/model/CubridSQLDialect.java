...
/**
 * Cubrid data source info
 */
public class CubridSQLDialect extends JDBCSQLDialect {
    ...
    protected CubridSQLDialect(String name, String id) {
        super(name, id);
    }
    ...
    public void initDriverSettings(JDBCSession session, JDBCDataSource dataSource, JDBCDatabaseMetaData metaData) {
        ...
        String delimitersString = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_SCRIPT_DELIMITER));
        ...
        String escapeStr = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_STRING_ESCAPE_CHAR));
        if (!CommonUtils.isEmpty(escapeStr)) {
            this.stringEscapeCharacter = escapeStr.charAt(0);
        }
        this.scriptDelimiterRedefiner =
            CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_SCRIPT_DELIMITER_REDEFINER), null);
        this.hasDelimiterAfterQuery = CommonUtils.toBoolean(driver.getDriverParameter(CubridConstants.PARAM_SQL_DELIMITER_AFTER_QUERY));
        this.hasDelimiterAfterBlock = CommonUtils.toBoolean(driver.getDriverParameter(CubridConstants.PARAM_SQL_DELIMITER_AFTER_BLOCK));
        this.legacySQLDialect = CommonUtils.toBoolean(driver.getDriverParameter(CubridConstants.PARAM_LEGACY_DIALECT));
        this.supportsUpsert = ((CubridDataSource)dataSource).getMetaModel().supportsUpsertStatement();
        if (this.supportsUpsert) {
            addSQLKeyword("UPSERT");
        }

        this.supportDelimiterInViews =
            CommonUtils.toBoolean(driver.getDriverParameter(CubridConstants.PARAM_SUPPORTS_DELIMITER_IN_VIEWS));

        String driverUnquotedCase = CommonUtils.toString(
            driver.getDriverParameter(CubridConstants.PARAM_STORED_UNQUOTED_CASE),
            null);
        ...
        String driverQuotedCase = CommonUtils.toString(
            driver.getDriverParameter(CubridConstants.PARAM_STORED_QUOTED_CASE),
            null);
        ...
        this.useSearchStringEscape = CommonUtils.getBoolean(driver.getDriverParameter(CubridConstants.PARAM_USE_SEARCH_STRING_ESCAPE), false);

        this.quoteReservedWords = CommonUtils.getBoolean(driver.getDriverParameter(CubridConstants.PARAM_QUOTE_RESERVED_WORDS), true);
        this.testSQL = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_QUERY_PING));
        if (CommonUtils.isEmpty(this.testSQL)) {
            this.testSQL = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_QUERY_GET_ACTIVE_DB));
        }
        this.dualTable = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_DUAL_TABLE));
        if (this.dualTable.isEmpty()) {
            this.dualTable = null;
        }
        this.omitCatalogName = CommonUtils.toBoolean(driver.getDriverParameter(CubridConstants.PARAM_OMIT_CATALOG_NAME));
        this.supportsMultiInsert = CommonUtils.toBoolean(driver.getDriverParameter(CubridConstants.PARAM_SUPPORTS_MULTI_INSERT));

        final String identifierQuotes = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_IDENTIFIER_QUOTES));
