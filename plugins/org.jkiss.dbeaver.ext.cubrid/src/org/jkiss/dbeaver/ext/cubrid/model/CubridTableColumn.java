...
public class CubridTableColumn extends JDBCTableColumn<CubridTableBase> implements DBSTableColumn, DBPNamedObject2, JDBCColumnKeyType, DBPOrderedObject
{
    ...
    {
        final Collection<CubridUniqueKey> uniqueKeysCache = getTable().getContainer().getConstraintKeysCache().getCachedObjects(getTable());
        if (!CommonUtils.isEmpty(uniqueKeysCache)) {
            for (CubridUniqueKey key : uniqueKeysCache) {
