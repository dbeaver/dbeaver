...
/**
 * IoTDBConnectionPage (mostly from GenericConnectionPage, use server as sql dialect)
 */
public class IoTDBConnectionPage extends ConnectionPageWithAuth implements IDialogPageProvider {
    private static final Log log = Log.getLog(IoTDBConnectionPage.class);
    ...
    private static final String GROUP_SERVER = "sqlDialect";
    ...
    public void createControl(Composite composite) {
        ...
        settingsGroup = UIUtils.createControlGroup(addrGroup, IoTDBUIMessages.dialog_connection_general_tab, 4, GridData.FILL_HORIZONTAL, 0);
...

        {
            ...
            Label urlLabel = UIUtils.createControlLabel(settingsGroup, IoTDBUIMessages.dialog_connection_jdbc_url_);
...

        {
            ...
            hostLabel.setText(IoTDBUIMessages.dialog_connection_host_label);
            ...
            portLabel.setText(IoTDBUIMessages.dialog_connection_port_label);
        ...
        {
            sqlDialectCombo = UIUtils.createLabelCombo(settingsGroup, "Sql Dialect", SWT.DROP_DOWN | SWT.READ_ONLY);
            sqlDialectCombo.addModifyListener(textListener);

            Control emptyLabel = UIUtils.createEmptyLabel(settingsGroup, 2, 1);

            addControlToGroup(GROUP_SERVER, sqlDialectCombo);
...

        {
            ...
            String dbTerm = (String) driver.getDriverParameter("term-database");
            if (CommonUtils.isNotEmpty(dbTerm)) {
                ...
                dbLabel.setText(IoTDBUIMessages.dialog_connection_database_schema_label);
            }
...

        {
            ...
            pathLabel.setText(IoTDBUIMessages.dialog_connection_path_label);
            ...
            UIUtils.createDialogButton(buttonsPanel, IoTDBUIMessages.dialog_connection_browse_button, null, IoTDBUIMessages.dialog_connection_browse_button_tip, new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
            ...
            if (CommonUtils.toBoolean(site.getDriver().getDriverParameter("supports-embedded-database-creation"))) {
                gl.numColumns += 1;
                UIUtils.createDialogButton(
                    buttonsPanel,
                    IoTDBUIMessages.dialog_connection_create_button,
                    null,
                    IoTDBUIMessages.dialog_connection_create_button_tip,
    ...
    private String showDatabaseFileSelectorDialog(int style) {
        if (metaURL.getAvailableProperties().contains(DBConstants.PROP_FILE)) {
            ...
            dialog.setText(IoTDBUIMessages.dialog_connection_db_file_chooser_text);
            return dialog.open();
        } else {
            ...
            dialog.setText(IoTDBUIMessages.dialog_connection_db_folder_chooser_text);
            dialog.setMessage(IoTDBUIMessages.dialog_connection_db_folder_chooser_message);
            return dialog.open();
        }
    }
    ...
    public void loadSettings() {
        ...
        UIUtils.asyncExec(() -> {
            if (CommonUtils.isEmpty(site.getDriver().getSampleURL())) {
                ...
                hostText.setFocus();
            } else if (sqlDialectCombo != null && sqlDialectCombo.isVisible()) {
    ...
    private void parseSampleURL(DBPDriver driver) {
        ...
        if (!useCustomUrl) {
            ...
            boolean isSampleUrlUsable = properties.contains(DBConstants.PROP_HOST) ||
                ...
                properties.contains("sqlDialect") ||
            ...
            if (isSampleUrlUsable) {
                ...
                showControlGroup(GROUP_SERVER, properties.contains("sqlDialect"));
    ...
    private boolean canCreateEmbeddedDatabase() {
        final String param = CommonUtils.toString(site.getDriver().getDriverParameter("create-url-param"));
        return !CommonUtils.isEmpty(param) && !CommonUtils.isEmptyTrimmed(pathText.getText());
    }

    private void createEmbeddedDatabase() {
        String paramCreate = CommonUtils.toString(site.getDriver().getDriverParameter("create-url-param"));
