...
/**
 * Doris DataSource - extends GenericDataSource with Doris-specific catalog loading
 */
public class DorisDataSource extends GenericDataSource {
    ...
    protected JDBCExecutionContext createExecutionContext(JDBCRemoteInstance instance, String type) {
        return new DorisExecutionContext(instance, type);
    }
    ...
    public List<String> getCatalogsNames(
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load Doris catalogs")) { //$NON-NLS-1$
            try (JDBCPreparedStatement dbStat = session.prepareStatement("SHOW CATALOGS")) { //$NON-NLS-1$
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    while (dbResult.next()) {
                        String catalogName = JDBCUtils.safeGetString(dbResult, COL_CATALOG_NAME);
            ...
            throw new DBException("Error loading Doris catalogs", e);
        }
        return catalogNames;
    }
