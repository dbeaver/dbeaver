...
/**
 * Doris Execution Context - manages active catalog and database context.
 * Supports 2-level context: Catalog and Database (schema).
 */
public class DorisExecutionContext extends JDBCExecutionContext
        implements DBCExecutionContextDefaults<DorisCatalog, DorisDatabase> {

    private static final Log log = Log.getLog(DorisExecutionContext.class);

    private String activeCatalogName;
    private String activeDatabaseName;

    DorisExecutionContext(@NotNull JDBCRemoteInstance instance, @NotNull String purpose) {
    ...
    @NotNull
    @Override
    public DorisExecutionContext getContextDefaults() {
        return this;
    }
    ...
    @Nullable
    @Override
    public DorisDatabase getDefaultSchema() {
        DorisCatalog catalog = getDefaultCatalog();
        if (catalog == null) {
            return null;
        }
    ...
    public void setDefaultCatalog(
        ...
        @Nullable DorisDatabase schema
    ) throws DBCException {
        if (catalog == null || isAlreadyActive(catalog, schema)) {
            return;
        }

        final DorisCatalog oldCatalog = getDefaultCatalog();
        final DorisDatabase oldSchema = getDefaultSchema();
    ...
    private boolean isAlreadyActive(@NotNull DorisCatalog catalog, @Nullable DorisDatabase schema) {
    ...
    private void fireSelectionChangeEvents(
        @Nullable DorisCatalog oldCatalog,
        @NotNull DorisCatalog newCatalog,
    ...
    @Override
    public void setDefaultSchema(@NotNull DBRProgressMonitor monitor, @Nullable DorisDatabase schema) throws DBCException {
        if (schema == null) {
            return;
        }

        // Ensure we're in the correct catalog first
        DorisCatalog catalog = schema.getCatalog();
    ...
    public boolean refreshDefaults(@NotNull DBRProgressMonitor monitor, boolean useBootstrapSettings) throws DBException {
        try (JDBCSession session = openSession(monitor, DBCExecutionPurpose.META, "Query active catalog and database")) { //$NON-NLS-1$
            ...
            // Get current catalog
            try (JDBCPreparedStatement dbStat = session.prepareStatement("SELECT CURRENT_CATALOG()")) { //$NON-NLS-1$
                ...
                activeCatalogName = DorisDataSource.DEFAULT_CATALOG_NAME;
            }
    ...
    private boolean setCurrentCatalog(@NotNull DBRProgressMonitor monitor, @NotNull String catalogName) throws DBCException {
        try (JDBCSession session = openSession(monitor, DBCExecutionPurpose.UTIL, "Set active catalog")) { //$NON-NLS-1$
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SWITCH " + DBUtils.getQuotedIdentifier(getDataSource(), catalogName))) { //$NON-NLS-1$
                dbStat.execute();
            } catch (SQLException e) {
