...
public class KingbaseValueParser {

    private static final Log log = Log.getLog(KingbaseValueParser.class);
    ...
    private static Object prepareToParseArray(DBCSession session, DBSTypedObject arrayType, String string) throws DBCException {
        ...
        try {
            ...
            if (componentType == null && arrayType instanceof KingbaseDataType && ((KingbaseDataType) arrayType).getTypeType() == KingbaseTypeType.d) {
                // Domains store component type information in another field
                componentType = ((KingbaseDataType) arrayType).getBaseType(session.getProgressMonitor());
            }
            if (componentType == null) {
                log.error("Can't get component type from array '" + arrayType.getFullTypeName() + "'");
                return string;
            } else {
                if (componentType instanceof KingbaseDataType) {
                    List<Object> itemStrings = parseArrayString(string, KingbaseUtils.getArrayDelimiter(arrayDataType));
                    return startTransformListOfValuesIntoArray(session, (KingbaseDataType) componentType, itemStrings);
                } else {
    ...
    { //transform into struct
        List<KingbaseDataTypeAttribute> attributes = CommonUtils.safeList(itemType.getAttributes(session.getProgressMonitor()));
    ...
    public static String[] parseSingleObject(String string) throws DBCException { //only for objects(structures), not for arrays
        ...
        try {
            ...
            throw new DBCException("Error parsing KBObject", e);
        }
    }
    ...
    public static String generateObjectString(Object[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            if (value instanceof Object[]) {
                String arrayKingbaseStyle = Arrays.deepToString((Object[]) value)
                        .replace("[", "{")
                        .replace("]", "}")
                        .replace(" ", "");
                line[i] = arrayKingbaseStyle; //Strings are not quoted
