...
/**
 * KingbaseGenericTypeCache
 */
@Deprecated
public class KingbaseGenericTypeCache extends JDBCBasicDataTypeCache<GenericStructContainer, JDBCDataType>
{
    private static final Log log = Log.getLog(KingbaseGenericTypeCache.class);

    private static String[] OID_TYPES = KingbaseDataType.getOidTypes();

    public KingbaseGenericTypeCache(GenericStructContainer owner) {
        super(owner);
    }
    ...
    {
        boolean supportsCategory = KingbaseUtils.supportsTypeCategory(session.getDataSource());
        return session.prepareStatement(
            "SELECT t.oid as typid,tn.nspname typnsname,t.* \n" +
                "FROM sys_catalog.sys_type t , sys_catalog.sys_namespace tn\n" +
    ...
    {
        boolean supportsTypeCategory = KingbaseUtils.supportsTypeCategory(session.getDataSource());
        String name = JDBCUtils.safeGetString(dbResult, "typname");
        if (CommonUtils.isEmpty(name)) {
            return null;
        }
        int typeLength = JDBCUtils.safeGetInt(dbResult, "typlen");
        KingbaseTypeCategory typeCategory = KingbaseTypeCategory.X;
        if (supportsTypeCategory) {
            try {
                typeCategory = KingbaseTypeCategory.valueOf(JDBCUtils.safeGetString(dbResult, "typcategory"));
            } catch (IllegalArgumentException e) {
                log.debug(e);
            }
        }
        int valueType;
        if (ArrayUtils.contains(OID_TYPES, name) || name.equals(KingbaseConstants.TYPE_HSTORE)) {
