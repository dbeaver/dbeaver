...
import org.jkiss.dbeaver.ext.kingbase.edit.KingbaseTableColumnManager;
 ...
 * KingbaseViewBase
 */
public abstract class KingbaseViewBase extends KingbaseTableReal implements DBSView {

    private String source;

    public KingbaseViewBase(KingbaseSchema catalog)
    {
        super(catalog);
    }

    public KingbaseViewBase(
        KingbaseSchema catalog,
        ResultSet dbResult)
    {
    ...
    public String getObjectDefinitionText(DBRProgressMonitor monitor, Map<String, Object> options) throws DBException
    {
            ...
            if (isPersisted()) {
                ...
                if (source == null) {
                    try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Read view definition")) {
                        ...
                        source = KingbaseUtils.getViewDDL(monitor, this, definition, options);
        ...
        if (CommonUtils.getOption(options, DBPScriptObject.OPTION_INCLUDE_COMMENTS)) {
            ...
            for (KingbaseTableColumn column : CommonUtils.safeCollection(getAttributes(monitor))) {
        ...
        if (isPersisted()) {
            Collection<KingbaseTrigger> triggers = getTriggers(monitor);
    ...
    private String fetchViewQueryResultIfItNull(JDBCSession session) throws SQLException, DBException {
        String definition = JDBCUtils.queryString(session, "SELECT sys_get_viewdef(" + getObjectId() + ", true)");
        if (definition == null) {
