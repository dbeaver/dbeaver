...
/**
 * KingbaseAccessMethod
 */
public class KingbaseAccessMethod extends KingbaseInformation {
    ...
    public KingbaseAccessMethod(KingbaseDatabase database, ResultSet dbResult)
        throws SQLException
    {
        super(database);
    ...
    @Association
    public Collection<KingbaseOperatorClass> getOperatorClasses(DBRProgressMonitor monitor) throws DBException {
        return operatorClassCache.getAllObjects(monitor, this);
    }

    public KingbaseOperatorClass getOperatorClass(DBRProgressMonitor monitor, long oid) throws DBException {
        return KingbaseUtils.getObjectById(monitor, operatorClassCache, this, oid);
    }

    @Association
    public Collection<KingbaseOperatorFamily> getOperatorFamilies(DBRProgressMonitor monitor) throws DBException {
        return operatorFamilyCache.getAllObjects(monitor, this);
    }

    public KingbaseOperatorFamily getOperatorFamily(DBRProgressMonitor monitor, long oid) throws DBException {
        return KingbaseUtils.getObjectById(monitor, operatorFamilyCache, this, oid);
    }

    static class OperatorClassCache extends JDBCObjectCache<KingbaseAccessMethod, KingbaseOperatorClass> {

        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull KingbaseAccessMethod owner)
                throws SQLException
        {
            return session.prepareStatement(
                    "SELECT oc.oid,oc.* FROM sys_catalog.sys_opclass oc " +
                            "\nORDER BY oc.oid"
            );
        }

        @Override
        protected KingbaseOperatorClass fetchObject(@NotNull JDBCSession session, @NotNull KingbaseAccessMethod owner, @NotNull JDBCResultSet dbResult)
                throws SQLException, DBException
        {
            return new KingbaseOperatorClass(owner, dbResult);
        }
    }

    static class OperatorFamilyCache extends JDBCObjectCache<KingbaseAccessMethod, KingbaseOperatorFamily> {
        ...
        {
            return session.prepareStatement(
                    "SELECT of.oid,of.* FROM sys_catalog.sys_opfamily of " +
                            "\nORDER BY of.oid"
            );
        }

        @Override
        protected KingbaseOperatorFamily fetchObject(@NotNull JDBCSession session, @NotNull KingbaseAccessMethod owner, @NotNull JDBCResultSet dbResult)
                throws SQLException, DBException
        {
            return new KingbaseOperatorFamily(owner, dbResult);
        }
    }
