 ...
 * KingbaseTrigger
 */
public class KingbaseTrigger extends KingbaseTriggerBase implements DBSEntityElement
{
    private static final Log log = Log.getLog(KingbaseTrigger.class);
    ...
    private KingbaseTableReal table;
    ...
    private KingbaseTriggerType type;
    private KingbaseTableColumn[] columnRefs;
    protected String description;
    private String body;

    public KingbaseTrigger(
        @NotNull DBRProgressMonitor monitor,
        @NotNull KingbaseTableReal table,
        ...
        if (CommonUtils.isBitSet(tgType, TRIGGER_TYPE_ROW)) {
            type = KingbaseTriggerType.ROW;
        } else {
            type = KingbaseTriggerType.STATEMENT;
        }

        Object attrNumbersObject = JDBCUtils.safeGetObject(dbResult, "tgattr");
        if (attrNumbersObject != null) {
            int[] attrNumbers = KingbaseUtils.getIntVector(attrNumbersObject);
            if (attrNumbers != null) {
                int attrCount = attrNumbers.length;
                columnRefs = new KingbaseTableColumn[attrCount];
                for (int i = 0; i < attrCount; i++) {
                    int colNumber = attrNumbers[i];
                    final KingbaseTableColumn attr = KingbaseUtils.getAttributeByNum(getTable().getAttributes(monitor), colNumber);
                    if (attr == null) {
    ...
    public KingbaseTrigger(@NotNull KingbaseTableReal parent, @NotNull String name) {
    ...
    public KingbaseTriggerType getType() {
        return type;
    }

    @Property(viewable = true, order = 5, valueRenderer = ColumnNameTransformer.class)
    public KingbaseTableColumn[] getColumnRefs() {
        return columnRefs;
    }
    ...
    @Override
    public KingbaseTableBase getTable()
    {
        return table;
    }
    ...
    public KingbaseProcedure getFunction(DBRProgressMonitor monitor) throws DBException {
        if (functionId == 0) {
            return null;
        }
    ...
    public void setFunction(KingbaseProcedure function) {
        if (function == null){
            this.functionId = 0;
    ...
    @NotNull
    @Override
    public KingbaseTableReal getParentObject()
    {
        return table;
    }
        ...
        if (CommonUtils.isEmpty(body)) {
            if (isPersisted()) {
                try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Read trigger definition")) {
                    body = JDBCUtils.queryString(session, "SELECT sys_catalog.sys_get_triggerdef(?)", objectId);
                    if (body != null) {
    ...
    public DBSObject refreshObject(@NotNull DBRProgressMonitor monitor) throws DBException {
        KingbaseTableReal.TriggerCache triggerCache = getParentObject().getTriggerCache();
    ...
    public void refreshObjectState(@NotNull DBRProgressMonitor monitor) throws DBCException {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Refresh triggers state")) {
            try {
                enabledState = JDBCUtils.queryString(session, "SELECT tgenabled FROM sys_catalog.sys_trigger WHERE oid=?", getObjectId());
            } catch (SQLException e) {
    ...
    public static class ColumnNameTransformer implements IPropertyValueTransformer {
        ...
        public Object transform(Object object, Object value) throws IllegalArgumentException {
            if (value instanceof KingbaseTableColumn[]) {
                StringBuilder sb = new StringBuilder();
                for (KingbaseTableColumn col : (KingbaseTableColumn[])value) {
