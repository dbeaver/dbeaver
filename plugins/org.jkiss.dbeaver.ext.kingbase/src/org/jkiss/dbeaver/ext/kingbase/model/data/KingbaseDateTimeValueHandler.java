...
/**
 * KingbaseDateTimeValueHandler.
 */
public class KingbaseDateTimeValueHandler extends JDBCDateTimeValueHandler {
    ...
    public KingbaseDateTimeValueHandler(DBDFormatSettings formatSettings) {
        ...
        if (value instanceof String) {
            try {
                ((JDBCPreparedStatement)statement).setObject(index + 1, value.toString(),
                    ((KingbaseDataSource)session.getDataSource()).getServerType().getParameterBindType(type, value));
            }
    ...
    protected String getFormatterId(DBSTypedObject column) {
        switch (column.getTypeName()) {
            case KingbaseConstants.TYPE_TIMETZ:
                return DBDDataFormatter.TYPE_NAME_TIME_TZ;
            case KingbaseConstants.TYPE_TIMESTAMPTZ:
