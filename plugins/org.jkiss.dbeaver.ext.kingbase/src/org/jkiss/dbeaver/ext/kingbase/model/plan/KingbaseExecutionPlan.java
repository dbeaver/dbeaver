...
/**
 * Kingbase execution plan analyser
 */
public class KingbaseExecutionPlan extends AbstractExecutionPlan {

    private static final Log log = Log.getLog(KingbaseExecutionPlan.class);
    ...
    public KingbaseExecutionPlan(boolean oldQuery, boolean verbose, String query, DBCQueryPlannerConfiguration configuration)
    {
    ...
    public KingbaseExecutionPlan(String query, List<KingbasePlanNodeExternal> nodes) {
        this.query = query;
    ...
    public String getPlanQueryString() {
        if (oldQuery) {
            ...
            for (Map.Entry<String, Object> entry : CommonUtils.safeCollection(parameters.entrySet())) {
                ...
                if (KingbaseQueryPlaner.PARAM_TIMING.equals(key)
                    && !CommonUtils.toBoolean(parameters.get(KingbaseQueryPlaner.PARAM_ANALYSE))
                ) {
                    continue;
                }
                if (KingbaseQueryPlaner.PARAM_COSTS.equals(key) || KingbaseQueryPlaner.PARAM_TIMING.equals(key)) {
    ...
    private void parsePlanXML(DBCSession session, SQLXML planXML) throws SQLException, XMLException {
        ...
        for (Element planElement : XMLUtils.getChildElementList(queryElement, "Plan")) {
            rootNodes.add(new KingbasePlanNodeXML(session.getDataSource(), null, planElement));
        }
    }

    private void parsePlanText(DBCSession session, List<String> lines) {
        DBPDataSource dataSource = session.getDataSource();
        List<KingbasePlanNodeText> nodes = new ArrayList<>(lines.size());
        KingbasePlanNodeText rootNode = null, curNode = null, curParentNode = null;
        int curIndent = 0;
        for (String line : lines) {
            ...
            if (curIndent == 0 && lineIndent == 0) {
                // Root node
                curParentNode = curNode = rootNode = new KingbasePlanNodeText(dataSource, null, line, lineIndent);
                nodes.add(rootNode);
                ...
                if (line.substring(lineIndent).startsWith(NODE_PREFIX)) {
                    ...
                    curNode = new KingbasePlanNodeText(dataSource, curParentNode, line, lineIndent);
                    nodes.add(0,curNode);
