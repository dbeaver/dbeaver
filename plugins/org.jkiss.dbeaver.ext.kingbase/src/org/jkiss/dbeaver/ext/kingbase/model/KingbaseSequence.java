 ...
 * KingbaseSequence
 */
public class KingbaseSequence extends KingbaseTableBase implements DBSSequence, DBPQualifiedObject
{
    private static final Log log = Log.getLog(KingbaseSequence.class);
    ...
    public static class AdditionalInfoValidator implements IPropertyCacheValidator<KingbaseSequence> {
        @Override
        public boolean isPropertyCached(KingbaseSequence object, Object propertyId)
        {
    ...
    public KingbaseSequence(KingbaseSchema schema, JDBCResultSet dbResult) {
        super(schema, dbResult);
    }

    public KingbaseSequence(KingbaseSchema catalog) {
        super(catalog);
    }
    ...
    public void loadAdditionalInfo(DBRProgressMonitor monitor) {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load sequence additional info")) {
            ...
            try (JDBCPreparedStatement dbSeqStat = session.prepareStatement(
                "SELECT * from sys_catalog.sys_sequences WHERE schemaname=? AND sequencename=?")) {
    ...
    public static class CacheAndCycleValidator implements IPropertyValueValidator<KingbaseSequence, Object> {
        @Override
        public boolean isValidValue(KingbaseSequence object, Object value) throws IllegalArgumentException {
