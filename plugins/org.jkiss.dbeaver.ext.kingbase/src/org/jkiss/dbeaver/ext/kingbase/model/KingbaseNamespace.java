 ...
 * KingbaseNamespace
 */
public class KingbaseNamespace implements DBSNamespace  {

    private static final Log log = Log.getLog(KingbaseNamespace.class);
    ...
    public KingbaseNamespace(@NotNull KingbaseSchema schema) {
        this.schema = schema;
    }
    ...
    public DBSObject getObjectByName(@NotNull DBRProgressMonitor monitor, @NotNull String name) throws DBException {
        try (JDBCSession session = DBUtils.openMetaSession(monitor, schema, "Search KB class")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT oid,relkind," + (schema.getDataSource().isSupportsReltypeColumn() ? "" : "0 as ") + "reltype " +
                    "FROM sys_catalog.sys_class WHERE relnamespace=? AND relname=?\n" +
                    "UNION ALL\n" +
                    "SELECT oid,'c',oid FROM sys_catalog.sys_type WHERE typnamespace=? AND typname=?")) {
