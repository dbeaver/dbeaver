 ...
 */
package org.jkiss.dbeaver.ext.kingbase.model.lock;
...
/**
 * Kingbase lock manager
 */

public class KingbaseLockManager extends LockGraphManager implements DBAServerLockManager<KingbaseLock,KingbaseLockItem> {
	...
	public static final String LOCK_QUERY =
        ...
        "where not l.granted "+
        ") la "+
        "join sys_catalog.sys_stat_activity blocked_activity  ON blocked_activity.pid = la.pid "+
        "join sys_catalog.sys_stat_activity blocking_activity  ON blocking_activity.pid = la.blocked "+
        "where la.rid = 1) "+
	...
	public static final String LOCK_ITEM_QUERY = "select "+
			...
			" lock.pid as pid, "+
			" lock.granted"+
			" from sys_catalog.sys_locks lock "+
			"   left join sys_catalog.sys_database db "+
    ...
    public KingbaseLockManager(KingbaseDataSource dataSource) {
    ...
    @Override
    public Map<Object, KingbaseLock> getLocks(DBCSession session, Map<String, Object> options) throws DBException {
        try {

            Map<Object, KingbaseLock> locks = new HashMap<>(10);

            try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement(LOCK_QUERY)) {
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {

                    while (dbResult.next()) {
                        KingbaseLock l = new KingbaseLock(dbResult);
                        locks.put(l.getId(), l);
                    }
                }
    ...
    @Override
    public void alterSession(DBCSession session, KingbaseLock lock, Map<String, Object> options) throws DBException {
        try {
            try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement("SELECT sys_catalog.sys_terminate_backend(?)")) {
    ...
    @Override
    public Class<KingbaseLock> getLocksType() {
        return KingbaseLock.class;
    }


    @Override
    public Collection<KingbaseLockItem> getLockItems(DBCSession session, Map<String, Object> options)
        throws DBException {
        try {

            List<KingbaseLockItem> locks = new ArrayList<>();
            ...
            try (JDBCPreparedStatement dbStat = ((JDBCSession) session).prepareStatement(LOCK_ITEM_QUERY)) {
                ...
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    ...
                    while (dbResult.next()) {
                        locks.add(new KingbaseLockItem(dbResult));
                    }
                }
            }
