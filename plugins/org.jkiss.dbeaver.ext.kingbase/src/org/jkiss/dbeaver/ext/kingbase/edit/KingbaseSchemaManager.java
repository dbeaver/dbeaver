 ...
 */
public class KingbaseSchemaManager extends SQLObjectEditor<KingbaseSchema, KingbaseDatabase> implements DBEObjectRenamer<KingbaseSchema> {
    ...
    @Nullable
    @Override
    public DBSObjectCache<KingbaseDatabase, KingbaseSchema> getObjectsCache(KingbaseSchema object)
    {
        return object.getDatabase().schemaCache;
    }

    @Override
    protected KingbaseSchema createDatabaseObject(@NotNull DBRProgressMonitor monitor, @NotNull DBECommandContext context, final Object container, Object copyFrom, @NotNull Map<String, Object> options)
    {
        KingbaseDatabase database = (KingbaseDatabase) container;
        return database.createSchemaImpl(database, "NewSchema", (KingbaseRole) null);
    }

    @Override
    protected void addObjectCreateActions(@NotNull DBRProgressMonitor monitor, @NotNull DBCExecutionContext executionContext, @NotNull List<DBEPersistAction> actions, @NotNull ObjectCreateCommand command, @NotNull Map<String, Object> options)
    {
        final KingbaseSchema schema = command.getObject();
        final StringBuilder script = new StringBuilder("CREATE SCHEMA " + DBUtils.getQuotedIdentifier(schema));
        try {
            final KingbaseRole owner = schema.getOwner(monitor);
            if (owner != null) {
    ...
    @Override
    public void renameObject(@NotNull DBECommandContext commandContext, @NotNull KingbaseSchema schema, @NotNull Map<String, Object> options, @NotNull String newName) throws DBException
    {
        processObjectRename(commandContext, schema, options, newName);
    }

    protected void addObjectExtraActions(@NotNull DBRProgressMonitor monitor, @NotNull DBCExecutionContext executionContext, @NotNull List<DBEPersistAction> actions, @NotNull NestedObjectCommand<KingbaseSchema, PropertyHandler> command, @NotNull Map<String, Object> options)
    {
        KingbaseSchema schema = command.getObject();
    ...
    private static class CreateSchemaAction extends SQLDatabasePersistActionAtomic {
        ...
        public CreateSchemaAction(KingbaseSchema schema, StringBuilder sql) {
