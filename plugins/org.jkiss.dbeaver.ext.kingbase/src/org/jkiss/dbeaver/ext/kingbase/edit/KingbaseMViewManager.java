 ...
 * KingbaseViewManager
 */
public class KingbaseMViewManager extends KingbaseViewManager {

    @Override
    protected KingbaseMaterializedView createDatabaseObject(@NotNull DBRProgressMonitor monitor, @NotNull DBECommandContext context, Object container, Object copyFrom, @NotNull Map<String, Object> options)
    {
        KingbaseSchema schema = (KingbaseSchema)container;
        KingbaseMaterializedView newMV = new KingbaseMaterializedView(schema);
    ...
    public void appendViewDeclarationPrefix(DBRProgressMonitor monitor, StringBuilder sqlBuf, KingbaseViewBase view) throws DBException {
        KingbaseMaterializedView mview = (KingbaseMaterializedView) view;
        String[] relOptions = mview.getRelOptions();
        if (!ArrayUtils.isEmpty(relOptions)) {
            sqlBuf.append("\nWITH(").append(String.join("," , relOptions)).append(")");
        }
        KingbaseTablespace tablespace = mview.getTablespace(monitor);
        if (tablespace  != null) {
    ...
    protected void addObjectModifyActions(@NotNull DBRProgressMonitor monitor, @NotNull DBCExecutionContext executionContext, @NotNull List<DBEPersistAction> actionList, @NotNull ObjectChangeCommand command, @NotNull Map<String, Object> options) throws DBException {
        final KingbaseMaterializedView mView = (KingbaseMaterializedView) command.getObject();
