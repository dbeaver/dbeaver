...
/**
 * KingbaseMaterializedView
 */
public class KingbaseMaterializedView extends KingbaseViewBase
{
    private boolean withData;
    private long tablespaceId;

    public KingbaseMaterializedView(KingbaseSchema catalog) {
        super(catalog);
    }

    public KingbaseMaterializedView(
    ...
    protected String readExtraDefinition(JDBCSession session, Map<String, Object> options) throws DBException {
        Collection<KingbaseIndex> indexes = getIndexes(session.getProgressMonitor());
        if (!CommonUtils.isEmpty(indexes)) {
            StringBuilder indexDefs = new StringBuilder("\n-- View indexes:\n");
            for (KingbaseIndex index : indexes) {
    ...
    @Property(viewable = true, editable = true, updatable = true, order = 20, listProvider = KingbaseTableBase.TablespaceListProvider.class)
