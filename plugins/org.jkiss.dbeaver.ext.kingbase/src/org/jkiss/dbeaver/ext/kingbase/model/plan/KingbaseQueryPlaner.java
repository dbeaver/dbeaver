 ...
 * KingbaseQueryPlaner
 */
public class KingbaseQueryPlaner extends AbstractExecutionPlanSerializer implements DBCQueryPlanner 
{
    ...
    public KingbaseQueryPlaner(KingbaseDataSource dataSource) {
    ...
    public void serialize(@NotNull Writer writer, @NotNull DBCPlan plan) throws IOException {
        serializeJson(writer, plan, dataSource.getInfo().getDriverName(), new DBCQueryPlannerSerialInfo() {
            ...
            public void addNodeProperties(DBCPlanNode node, JsonObject nodeJson) {
                ...
                if (node instanceof KingbasePlanNodeBase) {
                    KingbasePlanNodeBase<?> pgNode = (KingbasePlanNodeBase<?>) node;
    ...
    public DBCPlan deserialize(@NotNull Reader planData) throws IOException, InvocationTargetException {
        try {
            ...
            ExecutionPlanDeserializer<KingbasePlanNodeExternal> loader = new ExecutionPlanDeserializer<>();

            List<KingbasePlanNodeExternal> planNodes = loader.loadRoot(dataSource, jo,
                (datasource, node, parent) -> new KingbasePlanNodeExternal((KingbaseDataSource) datasource, node, parent));

            return new KingbaseExecutionPlan(query, planNodes);
