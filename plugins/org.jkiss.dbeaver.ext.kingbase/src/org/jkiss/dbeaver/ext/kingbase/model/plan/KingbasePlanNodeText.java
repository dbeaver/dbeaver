 ...
 */
public class KingbasePlanNodeText extends KingbasePlanNodeBase<KingbasePlanNodeText> {
    ...
    private void addAttr(Map<String, String> attributes,String attrName, String attrVal) {
        ...
        switch (attrName) {
        case "cost":
            addAttr(attributes,KingbasePlanNodeBase.ATTR_STARTUP_COST,KingbasePlanNodeBase.ATTR_TOTAL_COST,attrVal);
            break;
        case "rows":
            addAttr(attributes,KingbasePlanNodeBase.ATTR_PLAN_ROWS,KingbasePlanNodeBase.ATTR_ACTUAL_ROWS,attrVal);
            break;
    ...
    public KingbasePlanNodeText(DBPDataSource dataSource, KingbasePlanNodeText parent, String line, int indent) {
        ...
        if (tokens.length > 0) {
            ...
            if (operation.length() >= OPERATION_LEN_MIN && addInfo.length() > 0) {
                ...
                attributes.put(KingbasePlanNodeBase.ATTR_NODE_TYPE, addInfo); 
                
            } else {
            
                attributes.put(KingbasePlanNodeBase.ATTR_NODE_TYPE, operation);
    ...
    private void parseObjName(Map<String, String> attributes, String[] tokens, String operation) {
        ...
        if (objName != null) {
            ...
            if (OPERATION_TABLES.contains(operation)) {
               ...
               attributes.put(KingbasePlanNodeBase.ATTR_RELATION_NAME, objName);
               
            } else if (OPERATION_INDEXES.contains(operation)) {
                
                attributes.put(KingbasePlanNodeBase.ATTR_INDEX_NAME, objName);
                
            } else if (OPERATION_FUNCTION.contains(operation)) {
                
                attributes.put(KingbasePlanNodeBase.ATTR_FUNCTION_NAME, objName);
                
           } else {
               
               attributes.put(KingbasePlanNodeBase.ATTR_OBJECT_NAME, objName);
