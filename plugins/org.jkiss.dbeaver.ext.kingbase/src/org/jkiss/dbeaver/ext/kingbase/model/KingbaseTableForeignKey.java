...
/**
 * KingbaseTableForeignKey
 */
public class KingbaseTableForeignKey extends KingbaseTableConstraintBase<KingbaseTableForeignKeyColumn> implements DBSTableForeignKey
{
    private static final Log log = Log.getLog(KingbaseTableForeignKey.class);
    ...
    private KingbaseTableBase refTable;
    private List<KingbaseTableForeignKeyColumn> columns = new ArrayList<>();

    public KingbaseTableForeignKey(
        ...
        @NotNull DBSForeignKeyModifyRule updateRule)
    {
        ...
        this.refTable = refConstraint == null ? null : (KingbaseTableBase) refConstraint.getParentObject();
    ...
    @NotNull
    @Override
    @Property(viewable = true, specific = true, updatable = true, editable = true, listProvider = KingbaseConstraintModifyRuleListProvider.class, order = 55)
    ...
    @NotNull
    @Override
    @Property(viewable = true, specific = true, updatable = true, editable = true, listProvider = KingbaseConstraintModifyRuleListProvider.class, order = 56)
    ...
    @Nullable
    @Override
    public List<KingbaseTableForeignKeyColumn> getAttributeReferences(DBRProgressMonitor monitor) throws DBException {
        return columns;
    }

    @Override
    public void setAttributeReferences(List<KingbaseTableForeignKeyColumn> columns) throws DBException {
    ...
    void cacheAttributes(DBRProgressMonitor monitor, List<? extends KingbaseTableConstraintColumn> children, boolean secondPass) {
        ...
        columns.clear();
        columns.addAll((Collection<? extends KingbaseTableForeignKeyColumn>) children);

        final List<KingbaseAttribute> lst = new ArrayList<>(children.size());
        for (KingbaseTableConstraintColumn c : children) {
            lst.add(((KingbaseTableForeignKeyColumn)c).getReferencedColumn());
        }
        try {
    ...
    public void addColumn(KingbaseTableForeignKeyColumn column) {
        this.columns.add(column);
    }

    public static class KingbaseConstraintModifyRuleListProvider implements IPropertyValueListProvider<KingbaseTableForeignKey> {
        ...
        @Override
        public Object[] getPossibleValues(KingbaseTableForeignKey foreignKey)
        {
