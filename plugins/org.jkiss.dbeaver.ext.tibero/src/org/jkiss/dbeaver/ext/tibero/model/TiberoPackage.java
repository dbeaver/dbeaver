...
public class TiberoPackage extends OraclePackage implements DBSObjectContainer, DBSPackage, DBSProcedureContainer {
    ...
    private class ProceduresCache extends JDBCObjectCache<OraclePackage, TiberoProcedurePackaged> {
        ...
        @Override
        protected void invalidateObjects(DBRProgressMonitor monitor, OraclePackage owner, Iterator<TiberoProcedurePackaged> objectIter) {
            Map<String, TiberoProcedurePackaged> overloads = new HashMap<>();
            while (objectIter.hasNext()) {
                final TiberoProcedurePackaged proc = objectIter.next();
                if (CommonUtils.isEmpty(proc.getName())) {
                    objectIter.remove();
                    continue;
                }
                final TiberoProcedurePackaged overload = overloads.get(proc.getName());
                if (overload == null) {
