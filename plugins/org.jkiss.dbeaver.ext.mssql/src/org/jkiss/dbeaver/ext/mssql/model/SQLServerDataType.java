...
public class SQLServerDataType implements DBSDataType, SQLServerObject, DBPQualifiedObject, DBPScriptObject {
    ...
    public String getObjectDefinitionText(DBRProgressMonitor monitor, Map<String, Object> options) throws DBCException {
        ...
        if (!tableType) {
            ...
            if ((systemDataType.getDataKind() == DBPDataKind.STRING || typeName.equals(SQLServerConstants.TYPE_VARBINARY)
                    || (typeName.equals(SQLServerConstants.TYPE_SQL_VARIANT))) && maxLength < 8000 ) {
                sql.append("(").append(maxLength).append(")");
            } else if (typeName.equals(SQLServerConstants.TYPE_DECIMAL) || typeName.equals(SQLServerConstants.TYPE_NUMERIC)) {
                sql.append("(").append(precision).append(",").append(scale).append(")");
            } else if (typeName.equals(SQLServerConstants.TYPE_FLOAT)) {
                sql.append("(").append(precision).append(")");
            } else if (typeName.equals(SQLServerConstants.TYPE_TIME) || typeName.equals(SQLServerConstants.TYPE_DATETIME2) ||
             typeName.equals(SQLServerConstants.TYPE_DATETIMEOFFSET)) {
