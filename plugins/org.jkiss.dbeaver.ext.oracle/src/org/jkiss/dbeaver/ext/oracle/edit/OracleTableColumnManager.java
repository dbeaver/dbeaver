...
public class OracleTableColumnManager extends SQLTableColumnManager<OracleTableColumn, OracleTableBase>
    ...
    protected OracleTableColumn createDatabaseObject(
        ...
        final OracleTableColumn column = createColumn(table);

        if (canCopyColumnProperties(copyFrom)) {
            copyColumnProperties(column, (OracleTableColumn) copyFrom);
        } else {
            DBSDataType columnType = findBestDataType(table, getDefaultColumnTypeName());
    ...
    @NotNull
    protected OracleTableColumn createColumn(@NotNull OracleTableBase table) {
        return new OracleTableColumn(table);
    }

    @NotNull
    protected String getDefaultColumnTypeName() {
        return "varchar2";
    }

    protected boolean canCopyColumnProperties(@Nullable Object copyFrom) {
        return copyFrom instanceof OracleTableColumn;
    }

    protected void copyColumnProperties(
        @NotNull OracleTableColumn column,
        @NotNull OracleTableColumn source
    ) throws DBException {
        column.setName(source.getName());
        if (source.getDataType() != null) {
            column.setDataType(source.getDataType());
        }
        column.setTypeName(source.getTypeName());
        column.setMaxLength(source.getMaxLength());
        column.setPrecision(source.getPrecision());
        column.setScale(source.getScale());
        column.setValueType(source.getTypeID());
        column.setRequired(source.isRequired());
        column.setAutoGenerated(source.isAutoGenerated());
