...
import org.jkiss.dbeaver.ext.cdata.registry.*;
...
final class CDataActivationDialog extends TitleAreaDialog {
    ...
    protected void okPressed() {
        ...
        try {
            activatedLicense = UIUtils.runWithDialog(
                monitor -> CDataLicenseActivator.activate(monitor, driver, activationTarget, request)
            );
    ...
    private static String getErrorMessage(@NotNull DBException error) {
        if (error instanceof CDataLicenseActivationException activation) {
            String details = activation.getDetails();
            if (details != null) {
                return details;
            }
            return activation.getStatus().isUnknown()
                ? CDataUIMessages.activation_failed
                : CDataLicenseUIUtils.getStatusText(activation.getStatus());
        }
        return CommonUtils.notEmpty(error.getMessage());
    }
    ...
    @NotNull
    CDataLicenseType getSelectedType() {
        return selectedType;
    }
