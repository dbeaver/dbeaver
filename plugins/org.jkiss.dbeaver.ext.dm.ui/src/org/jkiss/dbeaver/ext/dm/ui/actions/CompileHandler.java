...
import org.jkiss.dbeaver.ext.dm.model.DmObjectPersistAction;
...
public class CompileHandler extends DmTaskHandler {

	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
		final IWorkbenchPart activePart = HandlerUtil.getActivePart(event);
		final List<DmSourceObject> objects = getSelectedObjects(event);
		if (!objects.isEmpty()) {
			final Shell activeShell = HandlerUtil.getActiveShell(event);
			if (objects.size() == 1) {
				final DmSourceObject unit = objects.get(0);
				...
				if (error != null) {
					...
					String errorTitle = unit.getName() + " 编译失败";
					if (sourceHost != null) {
					...
					String message = unit.getName() + " 编译成功";
					if (sourceHost != null) {
						sourceHost.setCompileInfo(message, true);
					}
					UIUtils.showMessageBox(activeShell, "完成", message, SWT.ICON_INFORMATION);
				}
			} else {
				DmCompilerDialog dialog = new DmCompilerDialog(activeShell, objects);
				dialog.open();
			}
		}
		return null;
	...
	public static boolean compileUnit(DBRProgressMonitor monitor, DBCCompileLog compileLog, DmSourceObject unit)
			throws DBCException {
	...
	private List<DmSourceObject> getSelectedObjects(ExecutionEvent event) {
		...
		if (currentSelection instanceof IStructuredSelection && !currentSelection.isEmpty()) {
			for (Iterator<?> iter = ((IStructuredSelection) currentSelection).iterator(); iter.hasNext();) {
				...
				final DmSourceObject sourceObject = RuntimeUtils.getObjectAdapter(element, DmSourceObject.class);
			...
			if (activePart instanceof IEditorPart) {
				if (((IEditorPart) activePart).getEditorInput() instanceof IDatabaseEditorInput) {
					DBSObject dbsObject = ((IDatabaseEditorInput) ((IEditorPart) activePart).getEditorInput())
							.getDatabaseObject();
					if (dbsObject instanceof DmSourceObject) {
						objects.add((DmSourceObject) dbsObject);
					}
				}
			}
		}
	...
	public void updateElement(UIElement element, Map parameters) {
		List<DmSourceObject> objects = getDmSourceObjects(element);
		if (!objects.isEmpty()) {
			if (objects.size() > 1) {
				element.setText("Compile " + objects.size() + " objects");
			} else {
				final DmSourceObject sourceObject = objects.get(0);
				String objectType = DmUtils.formatWord(sourceObject.getSourceType().name());
