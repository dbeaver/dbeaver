...
/**
 * {@code GRANT} dialog for a Mimer SQL databank privilege - collects the grantee, whether to
 * grant {@code TABLE} or {@code SEQUENCE} (the right to create tables resp. sequences in the
 * databank), and whether that privilege can be granted on to others in turn.
 *
 * @author Mimer Information Technology
 */
public class MimerCreateDatabankPrivilegePage extends BaseObjectEditPage {

    private static final Log log = Log.getLog(MimerCreateDatabankPrivilegePage.class);

    private final MimerDatabankPrivilege privilege;

    private String grantee = "";
    private String privilegeType = MimerDatabankPrivilege.PRIVILEGE_TYPES[0];
    private boolean grantable;

    public MimerCreateDatabankPrivilegePage(@NotNull MimerDatabankPrivilege privilege) {
    ...
    protected Control createPageContents(@NotNull Composite parent) {
        ...
        UIUtils.createLabelText(composite, "Databank", privilege.getDatabank().getName()).setEditable(false);

        Combo privilegeCombo = UIUtils.createLabelCombo(composite, "Privilege", SWT.DROP_DOWN | SWT.READ_ONLY);
        for (String type : MimerDatabankPrivilege.PRIVILEGE_TYPES) {
        ...
        // also allows PROGRAM idents and PUBLIC as grant targets - typing one in stays possible.
