...
/**
 * {@code GRANT USAGE} dialog for a Mimer SQL domain - same shape as {@link
 * MimerCreateSequencePrivilegePage}, only one privilege type exists so there's no picker.
 *
 * @author Mimer Information Technology
 */
public class MimerCreateDomainPrivilegePage extends BaseObjectEditPage {

    private static final Log log = Log.getLog(MimerCreateDomainPrivilegePage.class);

    private final MimerDomainPrivilege privilege;

    private String grantee = "";
    private boolean grantable;

    public MimerCreateDomainPrivilegePage(@NotNull MimerDomainPrivilege privilege) {
        super("Grant USAGE");
    ...
    protected Control createPageContents(@NotNull Composite parent) {
        ...
        UIUtils.createLabelText(composite, "Domain", privilege.getDomain().getName()).setEditable(false);
