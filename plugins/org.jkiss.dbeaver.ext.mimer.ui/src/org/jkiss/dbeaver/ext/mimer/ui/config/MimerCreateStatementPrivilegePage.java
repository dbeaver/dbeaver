...
/**
 * {@code GRANT EXECUTE ON STATEMENT} dialog for Mimer SQL - collects the grantee ident and
 * whether it should be able to grant that privilege on to others in turn. Same shape as {@link
 * MimerCreateProgramPrivilegePage}.
 *
 * @author Mimer Information Technology
 */
public class MimerCreateStatementPrivilegePage extends BaseObjectEditPage {

    private static final Log log = Log.getLog(MimerCreateStatementPrivilegePage.class);

    private final MimerStatementPrivilege privilege;

    private String grantee = "";
    private boolean grantable;

    public MimerCreateStatementPrivilegePage(@NotNull MimerStatementPrivilege privilege) {
        super("Grant execute");
    ...
    protected Control createPageContents(@NotNull Composite parent) {
        ...
        UIUtils.createLabelText(composite, "Statement", privilege.getStatement().getName()).setEditable(false);
    ...
    private String[] loadIdentNames() {
        try {
            ...
            log.debug("Can't load ident list for the grant-execute-on-statement dialog", e);
        }
        return new String[0];
    }
