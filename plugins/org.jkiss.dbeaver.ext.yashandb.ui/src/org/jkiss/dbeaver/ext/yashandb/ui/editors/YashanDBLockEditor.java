...
import org.jkiss.dbeaver.ext.yashandb.model.lock.YashanDBLock;
import org.jkiss.dbeaver.ext.yashandb.model.lock.YashanDBLockManager;
...
public class YashanDBLockEditor extends AbstractLockEditor {

    @Override
    protected LockManagerViewer createLockViewer(DBCExecutionContext executionContext, Composite parent) {

        DBAServerLockManager<DBAServerLock, DBAServerLockItem> lockManager = (DBAServerLockManager) new YashanDBLockManager((YashanDBDataSource) executionContext.getDataSource());
        ...
        return new LockManagerViewer(this, parent, lockManager) {
            ...
            protected void onLockSelect(final DBAServerLock lock) {
                ...
                if (lock != null) {
                    final YashanDBLock pLock = (YashanDBLock) lock;
                    super.refreshDetail(new HashMap<String, Object>() {{
                        put(YashanDBLockManager.sidHold, pLock.getHold_sid());
                        put(YashanDBLockManager.sidWait, pLock.getWait_sid());
                    }});
                }
            }
        };
