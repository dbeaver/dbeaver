...
public class YashanDBSessionEditor extends AbstractSessionEditor {
    ...
    protected SessionManagerViewer createSessionViewer(DBCExecutionContext executionContext, Composite parent) {
        return new SessionManagerViewer<YashanDBServerSession>(this, parent, new YashanDBServerSessionManager((YashanDBDataSource) executionContext.getDataSource())) {
...
//                contributionManager.add(disconnectSessionAction);
                ...
                contributionManager.add(ActionUtils.makeActionContribution(
                        new Action(YashanDBUIMessages.views_session_manager_viewer_show_background, Action.AS_CHECK_BOX) {
                            {
                ...
                contributionManager.add(ActionUtils.makeActionContribution(
                        new Action(YashanDBUIMessages.views_session_manager_viewer_show_inactive, Action.AS_CHECK_BOX) {
                            {
                                setImageDescriptor(DBeaverIcons.getImageDescriptor(UIIcon.CONFIGURATION));
                                setToolTipText(YashanDBUIMessages.views_session_manager_viewer_show_inactive_sessions_tip);
    ...
    private class DisconnectSessionAction extends Action {
        ...
        DisconnectSessionAction(boolean kill) {
            super(
                    kill ? YashanDBUIMessages.editors_yashandb_session_editor_title_kill_session : YashanDBUIMessages.editors_yashandb_session_editor_title_disconnect_session,
        ...
        public void run() {
            ...
            final String action = (kill ? YashanDBUIMessages.editors_yashandb_session_editor_action_kill : YashanDBUIMessages.editors_yashandb_session_editor_action_disconnect) + YashanDBUIMessages.editors_yashandb_session_editor_action__session;
            ConfirmationDialog dialog = new ConfirmationDialog(
                    getSite().getShell(),
                    action,
                    null,
                    NLS.bind(YashanDBUIMessages.editors_yashandb_session_editor_confirm_action, action.toLowerCase(), sessions),
                    MessageDialog.CONFIRM,
                    new String[]{IDialogConstants.YES_LABEL, IDialogConstants.NO_LABEL},
                    0,
                    YashanDBUIMessages.editors_yashandb_session_editor_confirm_title,
                    false);
            if (dialog.open() == IDialogConstants.YES_ID) {
                Map<String, Object> options = new HashMap<>();
                if (kill) {
                    options.put(YashanDBServerSessionManager.PROP_KILL_SESSION, kill);
                }
