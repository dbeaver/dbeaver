...
public class YashanDBForeignKeyConfigurator implements DBEObjectConfigurator<YashanDBTableForeignKey> {
    ...
    @Override
    public YashanDBTableForeignKey configureObject(DBRProgressMonitor monitor, Object table, YashanDBTableForeignKey foreignKey, Map<String, Object> options) {
        return UITask.run(() -> {
            EditForeignKeyPage editPage = new EditForeignKeyPage(
                    YashanDBUIMessages.edit_yashandb_foreign_key_manager_dialog_title,
                    foreignKey,
            ...
            for (EditForeignKeyPage.FKColumnInfo tableColumn : editPage.getColumns()) {
                foreignKey.addColumn(
                        new YashanDBTableForeignKeyColumn(
                                foreignKey,
                                (YashanDBTableColumn) tableColumn.getOwnColumn(),
                                colIndex++));
            }
            return foreignKey;
        });
