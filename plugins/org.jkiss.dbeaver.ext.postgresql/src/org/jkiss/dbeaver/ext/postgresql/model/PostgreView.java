...
public class PostgreView extends PostgreTableBase
{
    ...
    public static class AdditionalInfoValidator implements IPropertyCacheValidator<PostgreView> {
        @Override
        public boolean isPropertyCached(PostgreView object, Object propertyId)
        {
    ...
    private void loadAdditionalInfo(DBRProgressMonitor monitor) throws DBCException
    {
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, getDataSource(), "Load table status")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                "SELECT * FROM " + PostgreConstants.META_TABLE_VIEWS + " WHERE " + PostgreConstants.COL_TABLE_SCHEMA + "=? AND " + PostgreConstants.COL_TABLE_NAME + "=?")) {
                dbStat.setString(1, getContainer().getName());
                dbStat.setString(2, getName());
                try (JDBCResultSet dbResult = dbStat.executeQuery()) {
                    if (dbResult.next()) {
                        try {
                            additionalInfo.setCheckOption(CheckOption.valueOf(JDBCUtils.safeGetString(dbResult, PostgreConstants.COL_CHECK_OPTION)));
                        } catch (IllegalArgumentException e) {
                            log.warn(e);
                        }
                        additionalInfo.setDefiner(JDBCUtils.safeGetString(dbResult, PostgreConstants.COL_DEFINER));
                        additionalInfo.setDefinition(
                            SQLUtils.formatSQL(
                                getDataSource(),
                                JDBCUtils.safeGetString(dbResult, PostgreConstants.COL_VIEW_DEFINITION)));
                        additionalInfo.setUpdatable("YES".equals(JDBCUtils.safeGetString(dbResult, PostgreConstants.COL_IS_UPDATABLE)));
                    }
                    additionalInfo.loaded = true;
                }
            }
