...
public class DataFormatterProfile implements DBDDataFormatterProfile, DBPPreferenceListener {
    ...
    private static final Pattern ID_SUFFIX_PATTERN = Pattern.compile("[A-Za-z]*[a-z0-9]+(?:Id|ID)$");
    ...
    private boolean isIdLike(@NotNull DBSTypedObject type) {
        ...
        if (type instanceof DBSAttributeBase attr) {
            if (attr.isAutoGenerated()) {
                return true;
            }

            String attrName = attr.getName();
            if (ID_SUFFIX_PATTERN.matcher(attrName).matches()) {
                return true;
            }

            String n = attrName.toLowerCase(Locale.ROOT);
            boolean looksLikeIdName =
                n.equals("id")
                    || n.endsWith("_id")
                    || n.startsWith("id_")
                    || n.endsWith("_key")
            ...
            if (looksLikeIdName && !looksLikeNonIdMetric) {
                return true;
            }
        }
        ...
        boolean noScale = scale == null || scale == 0;

        return noScale && precision != null && precision >= 8;
    }
