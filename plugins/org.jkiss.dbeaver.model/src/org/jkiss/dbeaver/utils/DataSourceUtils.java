...
public class DataSourceUtils {
    ...
    public static DBPDataSourceContainer getDataSourceBySpec(
        ...
        boolean createNewDataSourceIfNotExists
    ) {
        BySpecInfo supplier = getDataSourceSupplierBySpec(
            project,
            connectionSpec,
            parameterHandler,
            searchByParameters,
            createNewDataSourceIfNotExists
        );

        return supplier == null ? null : supplier.supplier().get();
    }

    public record BySpecInfo(
        boolean foundExisting,
        Supplier<DBPDataSourceContainer> supplier
    ) {
    }
    ...
    public static BySpecInfo getDataSourceSupplierBySpec(
        ...
        class DataSourceSupplierContext {
            boolean createNewDataSource = createNewDataSourceIfNotExists;
            ...
            public void parseConnectionSpec() {
            ...
            @Nullable
            public DBPDataSourceContainer findExistingDataSource() {
            ...
            @Nullable
            public DBPDataSourceContainer createNewDataSource() {
                if (driverName == null) {
                ...
                if (folderPath != null) {
                    DBPDataSourceFolder folder = dsRegistry.getFolder(folderPath);
                    if (folder != null) {
        ...
        DataSourceSupplierContext ctx = new DataSourceSupplierContext();
        ctx.parseConnectionSpec();

        DBPDataSourceContainer existing = ctx.findExistingDataSource();
        if (existing != null) {
            return new BySpecInfo(true, () -> existing);
        } else {
            if (ctx.createNewDataSource) {
                // delay creation when needed
                return new BySpecInfo(false, ctx::createNewDataSource);
            } else {
                return null;
            }
        }
