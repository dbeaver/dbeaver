...
class PreviewMappingDialog extends BaseDialog {
    ...
    PreviewMappingDialog(Shell parentShell, DataTransferPipe pipe, DatabaseMappingContainer mappingContainer, DatabaseConsumerSettings consumerSettings) {
        ...
        try {
            getWizard().getContainer().run(true, true, mon -> {
                ...
                try {
                    ...
                    getProducerSettings().updateMappingsFromStream(monitor, getWizard().getSettings());
                    monitor.worked(1);
