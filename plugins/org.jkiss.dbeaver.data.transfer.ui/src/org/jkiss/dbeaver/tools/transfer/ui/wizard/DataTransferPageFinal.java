    ...
    protected boolean determinePageCompletion() {
        for (DataTransferPipe pipe : getWizard().getSettings().getDataPipes()) {
            if (pipe.getProducer() == null || !pipe.getProducer().isConfigurationComplete()) {
                setErrorMessage(NLS.bind(DTUIMessages.data_transfer_error_source_not_specified, pipe.getConsumer().getObjectName()));
                return false;
            }
            if (pipe.getConsumer() == null || !pipe.getConsumer().isConfigurationComplete()) {
                setErrorMessage(NLS.bind(DTUIMessages.data_transfer_error_target_not_specified, pipe.getProducer().getObjectName()));
                return false;
            }
        }
        return activated;
