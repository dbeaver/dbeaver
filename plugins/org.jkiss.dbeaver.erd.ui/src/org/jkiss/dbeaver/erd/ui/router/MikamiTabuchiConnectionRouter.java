...
public class MikamiTabuchiConnectionRouter extends AbstractRouter {
    ...
    private final FigureListener figureListener = source -> {
        ...
        if (MikamiTabuchiConnectionRouter.this.algorithm.updateObstacle(MikamiTabuchiConnectionRouter.this.figuresToBounds.get(
            source), newBounds)) {
    ...
    public void route(Connection connection) {
        ...
        UIUtils.asyncExec(() -> {
            ...
            for (OrthogonalPath path : updated) {
                ...
                Dimension tolerance = new Dimension(algorithm.getSpacing(), 0);
                tolerance = (Dimension) MapModeUtil.getMapMode(current).DPtoLP(tolerance);
                ...
                normalizeToStraightLineTolerance(points, tolerance.width);
    ...
    /**
     * Normalizing for GMF presentations, otherwise zooming will break
     * drawn lines
     * See {@link org.eclipse.gmf.runtime.draw2d.ui.internal.routers.RectilinearRouter)}
     *
     * @param line line to normalize
     * @param tolerance value before which line will be updates
     */
