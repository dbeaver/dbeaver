...
public class AltibaseMetaModel extends GenericMetaModel
{
    ...
    */
    ...
    public String getPackageDDL(DBRProgressMonitor monitor, AltibasePackage sourceObject, int package_type) throws DBException {
    	String ddl = null;
    	
    	if (DBMS_METADATA) {
    		ddl = getDDLFromDbmsMetadata(monitor, sourceObject, sourceObject.getSchema().getName(), 
    				(package_type == AltibaseConstants.PACKAGE_SPEC)?"PACKAGE_SPEC":"PACKAGE_BODY");
    	}
    	...
    	if (!DBMS_METADATA || AltibaseUtils.isEmpty(ddl)) {
	    	String sql = "SELECT "
	    			...
	    			+ " AND PP.PACKAGE_TYPE = " + package_type
    ...
    */
    ...
    @Override
    public GenericUniqueKey createConstraintImpl(GenericTableBase table, String constraintName, DBSEntityConstraintType constraintType, 
    		JDBCResultSet dbResult, boolean persisted) {
    	String condition = constraintType.equals(DBSEntityConstraintType.CHECK)? JDBCUtils.safeGetString(dbResult, "CHECK_CONDITION"):"";
