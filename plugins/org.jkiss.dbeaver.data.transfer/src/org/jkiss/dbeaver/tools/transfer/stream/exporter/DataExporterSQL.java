...
public class DataExporterSQL extends StreamExporterAbstract {
    ...
    private static final String PROP_UPDATE_OR = "keywordUpdate";
    private static final String PROP_DUPLICATE_KEY = "insertOnDuplicateKey";
    ...
    private final String KEYWORD_UPDATE_OR = "UPDATE OR";
    private final String KEYWORD_DUPLICATE_KEY = "ON DUPLICATE KEY UPDATE";
    private final String KEYWORD_ON_CONFLICT = "ON CONFLICT";
    private DBPIdentifierCase identifierCase;
    private boolean useKeywordUpdate;
    private static String duplicateKeyExpression;
    ...
    public void init(IStreamDataExporterSite site) throws DBException {
        ...
        useKeywordUpdate = CommonUtils.toBoolean(properties.get(PROP_UPDATE_OR));
        duplicateKeyExpression = CommonUtils.toString(properties.get(PROP_DUPLICATE_KEY));
    ...
    public void exportRow(DBCSession session, DBCResultSet resultSet, Object[] row) throws DBException, IOException {
            ...
            if (rowCount > 0) {
                if (insertMode == SQLDialect.MultiValueInsertMode.PLAIN) {
                    ...
                    if (!CommonUtils.isEmpty(onConflictExpression)) {
                        printExtraKeywordsAtTheEnd(out, KEYWORD_ON_CONFLICT, onConflictExpression);
                    } else if (!CommonUtils.isEmpty(duplicateKeyExpression)) {
                        printExtraKeywordsAtTheEnd(out, KEYWORD_DUPLICATE_KEY, duplicateKeyExpression);
                    }
                    sqlBuffer.append(";");
                }
                if (lineBeforeRows) {
        ...
        if (!CommonUtils.isEmpty(onConflictExpression) && insertMode == SQLDialect.MultiValueInsertMode.NOT_SUPPORTED) {
            printExtraKeywordsAtTheEnd(out, KEYWORD_ON_CONFLICT, onConflictExpression);
        } else if (!CommonUtils.isEmpty(duplicateKeyExpression) && insertMode == SQLDialect.MultiValueInsertMode.NOT_SUPPORTED) {
            printExtraKeywordsAtTheEnd(out, KEYWORD_DUPLICATE_KEY, duplicateKeyExpression);
        }
        if (insertMode == SQLDialect.MultiValueInsertMode.NOT_SUPPORTED) {
            out.write(";");
    ...
    private void printExtraKeywordsAtTheEnd(PrintWriter out, String keyword, String keywordExpression) {
        out.write(" " + identifierCase.transform(keyword) + " " + keywordExpression);
    }
