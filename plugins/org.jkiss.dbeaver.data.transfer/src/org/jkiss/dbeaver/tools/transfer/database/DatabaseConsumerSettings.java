...
public class DatabaseConsumerSettings implements IDataTransferConsumerSettings {
    ...
    public void saveContainerExtraSettings(
        @NotNull DatabaseMappingContainer container,
        @NotNull Map<String, Object> settings
    ) {
    }

    public void loadContainerExtraSettings(
        @NotNull DatabaseMappingContainer container,
        @NotNull Map<String, Object> settings
    ) {
    }
    ...
    @NotNull
    public Collection<? extends DatabaseMappingObject> getExtraMappingObjects(
    ...
    public boolean hasExtraTargetStructure() {
        return false;
    }

    @NotNull
    public DBEPersistAction[] generateExtraTargetTableDDL(
        @NotNull DBRProgressMonitor monitor,
    ...
    public boolean hasPostTransferActions() {
        return false;
    }
    ...
    @Nullable
    public DBPDataSource getTargetDataSource(@NotNull DatabaseMappingObject attrMapping) {
    ...
    public void loadSettings(
        ...
        useTransactions = CommonUtils.getBoolean(settings.get(DTConstants.PROP_USE_TRANSACTIONS), useTransactions);
        onDuplicateKeyInsertMethodId = CommonUtils.toString(settings.get(DTConstants.PROP_ON_DUPLICATE_KEY_METHOD), onDuplicateKeyInsertMethodId);
        ...
        useBulkLoad = CommonUtils.getBoolean(settings.get(DTConstants.PROP_USE_BULK_LOAD), useBulkLoad);
        ...
        List<DataTransferPipe> dataPipes = dataTransferSettings.getDataPipes();
        {
            if (!dataPipes.isEmpty()) {
                IDataTransferConsumer<?, ?> consumer = dataPipes.getFirst().getConsumer();
                if (consumer instanceof DatabaseTransferConsumer databaseTransferConsumer) {
                    final DBSDataManipulator targetObject = databaseTransferConsumer.getTargetObject();
                    if (targetObject != null && targetObject.getParentObject() instanceof DBSObjectContainer targetObjectParent) {
                        this.container = targetObjectParent;
                    } else if (consumer.getTargetObjectContainer() instanceof DBSObjectContainer targetObjectContainer) {
                        this.container = targetObjectContainer;
                    }
                }
            }
        ...
        if (dataTransferSettings.isTaskRunning() || DTActivator.getDefault().getPreferences()
            .getBoolean(DTConstants.PREF_RECONNECT_TO_LAST_DATABASE)) {
    ...
    public void saveSettings(@NotNull Map<String, Object> settings) {
        ...
        settings.put(DTConstants.PROP_USE_TRANSACTIONS, useTransactions);
        ...
        settings.put(DTConstants.PROP_ON_DUPLICATE_KEY_METHOD, onDuplicateKeyInsertMethodId);
        ...
        settings.put(DTConstants.PROP_USE_BULK_LOAD, useBulkLoad);
    ...
    public String getSettingsSummary() {
        ...
        DTUtils.addSummary(summary, DTMessages.database_consumer_settings_option_on_duplicate_key_method_label,
            onDuplicateKeyInsertMethodId
        );
        DTUtils.addSummary(
            summary,
            DTMessages.database_consumer_settings_option_transfer_auto_generated_columns,
            transferAutoGeneratedColumns
        );
        DTUtils.addSummary(
            summary,
            DTMessages.database_consumer_settings_option_disable_referential_integrity,
                    ...
                    try {
                        ...
                        if (!CommonUtils.isEmpty(entityId)) {
                            ...
                            if (node instanceof DBNDatabaseNode dbnDatabaseNode) {
                                dbObject = dbnDatabaseNode.getObject();
                            }
                        }
                        if (dbObject instanceof DBPDataSourceContainer dataSourceContainer) {
