...
/**
 * KingbaseSQL ENUM value manager
 */
public class KingbaseEnumValueManager extends EnumValueManager {

    private static final Log log = Log.getLog(KingbaseEnumValueManager.class);
    ...
    protected List<String> getEnumValues(IValueController valueController) {
        ...
        KingbaseDataType dataType = null;
        if (attribute instanceof DBSDataType) {
            dataType = (KingbaseDataType) attribute;
        } else if (attribute instanceof DBSTypedObjectEx) {
            dataType = (KingbaseDataType) ((DBSTypedObjectEx) attribute).getDataType();
        }
        if (dataType == null) {
            return null;
        }
        KingbaseDataType finalDataType = dataType;
