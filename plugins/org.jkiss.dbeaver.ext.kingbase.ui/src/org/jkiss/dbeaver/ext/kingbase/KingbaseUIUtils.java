...
public class KingbaseUIUtils {

    public static void addCompatibilityInfoLabelForForks(
        @NotNull Composite composite,
        @NotNull AbstractNativeToolWizard wizard,
        @Nullable KingbaseDataSource dataSource
    ) {
        boolean showInfoLabel = false;
        if (dataSource != null && dataSource.getServerType() instanceof KingbaseServerKingbaseSQL) {
            showInfoLabel = true;
        } else if (!wizard.getSettings().getDatabaseObjects().isEmpty()) {
            Object databaseObject = wizard.getSettings().getDatabaseObjects().get(0);
            if (databaseObject instanceof KingbaseObject pObject &&
                !(pObject.getDataSource().getServerType() instanceof KingbaseServerKingbaseSQL)
                ) {
                showInfoLabel = true;
            }
        }
        if (showInfoLabel) {
            Control infoLabel = UIUtils.createWarningLabel(
                composite,
                KingbaseMessages.wizard_info_label_incompatible_tool,
                GridData.FILL_BOTH,
                1);
