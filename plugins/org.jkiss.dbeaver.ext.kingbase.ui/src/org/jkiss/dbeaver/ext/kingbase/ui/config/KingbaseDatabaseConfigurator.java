...
/**
 * Kingbase database configurator
 */
public class KingbaseDatabaseConfigurator implements DBEObjectConfigurator<KingbaseDatabase> {

    @Override
    public KingbaseDatabase configureObject(@NotNull DBRProgressMonitor monitor, @Nullable DBECommandContext commandContext, @Nullable Object dataSource, @NotNull KingbaseDatabase database, @NotNull Map<String, Object> options) {
        return new UITask<KingbaseDatabase>() {
            @Override
            protected KingbaseDatabase runTask() {
            	KingbaseCreateDatabaseDialog dialog = new KingbaseCreateDatabaseDialog(UIUtils.getActiveWorkbenchShell(), database.getDataSource());
