...
/**
 * KingbaseCreateExtensionDialog
 */
public class KingbaseCreateTablespaceDialog extends BaseDialog
{
    private static final String DIALOG_ID = "DBeaver.KingbaseCreateTablespaceDialog";

    private final KingbaseTablespace newTablespace;
    ...
    public KingbaseCreateTablespaceDialog(Shell parentShell, KingbaseTablespace tablespace) {
        super(parentShell, KingbaseMessages.dialog_create_tablespace_title, null);
    ...
    protected Composite createDialogArea(Composite parent) {
        ...
        UIUtils.createLabelText(group,
        		KingbaseMessages.dialog_create_tablespace_database,
            newTablespace.getDatabase().getName(),
            SWT.BORDER | SWT.READ_ONLY);

        final Text nameText = UIUtils.createLabelText(group, KingbaseMessages.dialog_create_tablespace_name, ""); //$NON-NLS-2$
        ...
        final Combo userCombo = UIUtils.createLabelCombo(group, KingbaseMessages.dialog_create_tablespace_owner, SWT.BORDER | SWT.DROP_DOWN | SWT.READ_ONLY);
        ...
        if (newTablespace.getDataSource().getServerType().supportsTablespaceLocation()) {
            final Text locText = UIUtils.createLabelText(group, KingbaseMessages.dialog_create_tablespace_loc, "");
        ...
        UIUtils.createControlLabel(group, KingbaseMessages.dialog_create_tablespace_options);
        ...
        new AbstractJob("Load users") {
            ...
            protected IStatus run(DBRProgressMonitor monitor) {
                try {
                    ...
                    final KingbaseRole dba = newTablespace.getDatabase().getDBA(monitor);
