...
public class TiDBMySQLDatabaseManager extends SQLObjectEditor<TiDBMySQLCatalog, TiDBMySQLDataSource> {
    ...
    @Override
    public DBSObjectCache<? extends DBSObject, TiDBMySQLCatalog> getObjectsCache(TiDBMySQLCatalog object) {
        return ((TiDBMySQLDataSource) object.getDataSource()).getTiDBCatalogCache();
    }

    @Override
    protected TiDBMySQLCatalog createDatabaseObject(DBRProgressMonitor monitor, DBECommandContext context,
                                                    Object container, Object copyFrom, Map<String, Object> options) {
        return new TiDBMySQLCatalog((TiDBMySQLDataSource) container, null);
    }

    @Override
    protected void addObjectCreateActions(DBRProgressMonitor monitor, DBCExecutionContext executionContext,
            List<DBEPersistAction> actions,
            SQLObjectEditor<TiDBMySQLCatalog, TiDBMySQLDataSource>.ObjectCreateCommand command,
            Map<String, Object> options) throws DBException {
        final TiDBMySQLCatalog catalog = command.getObject();
    ...
    protected void addObjectDeleteActions(DBRProgressMonitor monitor, DBCExecutionContext executionContext,
            List<DBEPersistAction> actions,
            SQLObjectEditor<TiDBMySQLCatalog, TiDBMySQLDataSource>.ObjectDeleteCommand command,
            Map<String, Object> options) throws DBException {
        actions.add(new SQLDatabasePersistAction("Drop schema", "DROP SCHEMA `" + command.getObject().getName() + "`"));
    }

    private void appendDatabaseModifiers(TiDBMySQLCatalog catalog, StringBuilder script) {
