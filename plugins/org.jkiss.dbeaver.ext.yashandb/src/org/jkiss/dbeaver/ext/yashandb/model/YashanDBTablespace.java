...
public class YashanDBTablespace extends YashanDBGlobalObject implements DBPRefreshableObject, DBPObjectStatistics {
    ...
    public YashanDBDataFile getFile(DBRProgressMonitor monitor, long relativeFileNo) throws DBException {
        // the view 'DBA_DATA_FILES' in YashanDB lack of field 'relative_fno', set default value to 'relative_fno'.
        if (fileCache.getAllObjects(monitor, this) != null) {
            return fileCache.getAllObjects(monitor, this).get(0);
        }
        return null;
    }
    ...
    private void loadSizes(DBRProgressMonitor monitor) throws DBException {
        try (final JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load tablespace '" + getName() + "' statistics")) {
            try (JDBCPreparedStatement dbStat = session.prepareStatement("SELECT * FROM\n" +
                    "(SELECT SUM(F.BYTES) AVAILABLE_SPACE FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "DBA_DATA_FILES F WHERE F.TABLESPACE_NAME=?) XDF,\n" +
                    "(SELECT SUM(S.BYTES) USED_SPACE FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "DBA_SEGMENTS S WHERE S.TABLESPACE_NAME=?) XS")) {
    ...
    static class FileCache extends JDBCObjectCache<YashanDBTablespace, YashanDBDataFile> {
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBTablespace owner) throws SQLException {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT * FROM " + YashanDBUtils.getSysSchemaPrefix(owner.getDataSource()) + "DBA_" +
        ...
        @Override
        protected YashanDBDataFile fetchObject(@NotNull JDBCSession session, @NotNull YashanDBTablespace owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
    ...
    static class SegmentCache extends JDBCObjectCache<YashanDBTablespace, YashanDBSegment<YashanDBTablespace>> {
        ...
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBTablespace owner) throws SQLException {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT * FROM " + YashanDBUtils.getSysUserViewName(session.getProgressMonitor(), owner.getDataSource(), "SEGMENTS") +
        ...
        @Override
        protected YashanDBSegment<YashanDBTablespace> fetchObject(@NotNull JDBCSession session, @NotNull YashanDBTablespace owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
            return new YashanDBSegment<>(session.getProgressMonitor(), owner, resultSet);
        }
    }


    static Object resolveTablespaceReference(DBRProgressMonitor monitor, DBSObjectLazy<YashanDBDataSource> referrer, @Nullable Object propertyId) throws DBException {
        ...
        if (!dataSource.isAdmin()) {
            ...
            return YashanDBUtils.resolveLazyReference(monitor, dataSource, dataSource.tablespaceCache, referrer, propertyId);
        }
    }

    public static class TablespaceReferenceValidator implements IPropertyCacheValidator<DBSObjectLazy<YashanDBDataSource>> {
        @Override
        public boolean isPropertyCached(DBSObjectLazy<YashanDBDataSource> object, Object propertyId) {
            return
                    object.getLazyReference(propertyId) instanceof YashanDBTablespace ||
