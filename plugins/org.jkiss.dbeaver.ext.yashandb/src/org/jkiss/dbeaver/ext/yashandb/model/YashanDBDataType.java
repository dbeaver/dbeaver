 ...
 * @Description:
 */
public class YashanDBDataType extends YashanDBObject<DBSObject> implements DBSDataType, DBSEntity,
        DBPQualifiedObject, YashanDBSourceObject, DBPScriptObjectExt {
   ...
   public static class TypeDesc {
    ...
    protected YashanDBDataType(DBSObject owner, ResultSet dbResult) {
        ...
        if (!CommonUtils.isEmpty(superTypeOwner)) {
            this.superType = new YashanDBLazyReference(
                    superTypeOwner,
        ...
        if (owner instanceof YashanDBDataSource && flagPredefined) {
            ...
            if (TYPE_CODE_COLLECTION.equals(this.typeCode)) {
                ...
                if (this.name.equals(YashanDBConstants.TYPE_NAME_XML) && owner.getName().equals(YashanDBConstants.SCHEMA_SYS)) {
    ...
    public DBSObject getParentObject() {
        return parent instanceof YashanDBSchema ?
                parent :
    ...
    public YashanDBDataType getSuperType(DBRProgressMonitor monitor) {
        return null;
    }
    ...
    @Override
    @Association
    public List<YashanDBDataTypeAttribute> getAttributes(@NotNull DBRProgressMonitor monitor)
            throws DBException {
    ...
    @Override
    public YashanDBDataTypeAttribute getAttribute(@NotNull DBRProgressMonitor monitor, @NotNull String attributeName) throws DBException {
    ...
    @Nullable
    @Association
    public Collection<YashanDBDataTypeMethod> getMethods(DBRProgressMonitor monitor)
            throws DBException {
    ...
    public YashanDBDataType getComponentType(@NotNull DBRProgressMonitor monitor)
            throws DBException {
    ...
    public static YashanDBDataType resolveDataType(DBRProgressMonitor monitor, YashanDBDataSource dataSource, String typeOwner, String typeName) {
        typeName = normalizeTypeName(typeName);
    ...
    private class AttributeCache extends JDBCObjectCache<YashanDBDataType, YashanDBDataTypeAttribute> {
        ...
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBDataType owner) throws SQLException {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT * FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "ALL_TYPE_ATTRS " +
        ...
        @Override
        protected YashanDBDataTypeAttribute fetchObject(@NotNull JDBCSession session, @NotNull YashanDBDataType owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
    ...
    private class MethodCache extends JDBCObjectCache<YashanDBDataType, YashanDBDataTypeMethod> {
        ...
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBDataType owner) throws SQLException {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                    //todo:yashandb目前没有ALL_METHOD_RESULTS视图，查询会报错。先只查询ALL_TYPE_METHODS视图
//                    "SELECT m.*,r.RESULT_TYPE_OWNER,RESULT_TYPE_NAME,RESULT_TYPE_MOD\n" +
//                            "FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "ALL_TYPE_METHODS m\n" +
//                            "LEFT OUTER JOIN " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "ALL_METHOD_RESULTS r ON r.OWNER=m.OWNER AND r.TYPE_NAME=m.TYPE_NAME AND r.METHOD_NAME=m.METHOD_NAME AND r.METHOD_NO=m.METHOD_NO\n" +
...
//                            "ORDER BY m.METHOD_NO"
        ...
        @Override
        protected YashanDBDataTypeMethod fetchObject(@NotNull JDBCSession session, @NotNull YashanDBDataType owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
