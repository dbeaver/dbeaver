 ...
 * @Date 2023/7/5 11:07
 */
public class YashanDBSchedulerJob extends YashanDBSchemaObject implements YashanDBStatefulObject, DBPScriptObjectExt, YashanDBSourceObject {
...
//        allowRunsInRestrictedMode = JDBCUtils.safeGetString(dbResult, "ALLOW_RUNS_IN_RESTRICTED_MODE");
    ...
    public YashanDBSchedulerJob(YashanDBSchema schema,String name){
...
//    @Association
//    public Collection<YashanDBSchedulerJobArgument> getArguments(DBRProgressMonitor monitor) throws DBException
//    {
//        return argumentsCache.getAllObjects(monitor, this);
//    }

//    static class ArgumentsCache extends JDBCObjectCache<YashanDBSchedulerJob,YashanDBSchedulerJobArgument> {
//
//        @NotNull
//        @Override
//        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBSchedulerJob job) throws SQLException
//        {
//            JDBCPreparedStatement dbStat = session.prepareStatement(
//                    "SELECT * FROM "+ YashanDBUtils.getSysSchemaPrefix(job.getDataSource()) + "ALL_SCHEDULER_JOB_ARGS " +
...
//
//        @Override
//        protected YashanDBSchedulerJobArgument fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchedulerJob job, @NotNull JDBCResultSet resultSet) throws SQLException, DBException
//        {
//            return new YashanDBSchedulerJobArgument(job, resultSet);
//        }
//
//    }
    ...
    public void refreshObjectState(DBRProgressMonitor monitor) {
        if (monitor != null) {
            ...
            try (final JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load action for " + YashanDBObjectType.JOB + " '" + this.getName() + "'")) {
                try (JDBCPreparedStatement dbStat = session.prepareStatement(
                        "SELECT STATE FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "ALL_SCHEDULER_JOBS " +
                                "WHERE OWNER=? AND JOB_NAME=? ")) {
    ...
    public DBEPersistAction[] getRunActions() {
        ...
        return new DBEPersistAction[] {
                new YashanDBObjectPersistAction(
                        YashanDBObjectType.JOB,
                        "Run Job",
                        runScript.toString()
                )};
    }
    ...
    public String getObjectDefinitionText(DBRProgressMonitor monitor, Map<String, Object> options) throws DBException {
        if (jobAction == null && monitor != null) {
            ...
            try (final JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load action for " + YashanDBObjectType.JOB + " '" + this.getName() + "'")) {
                try (JDBCPreparedStatement dbStat = session.prepareStatement(
                        "SELECT JOB_ACTION FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + "ALL_SCHEDULER_JOBS " +
                                "WHERE OWNER=? AND JOB_NAME=? ")) {
                    dbStat.setString(1, getOwner() );
                    dbStat.setString(2, getName());
                    dbStat.setFetchSize(DBConstants.METADATA_FETCH_SIZE);
        ...
        return jobAction==null?sourceDeclaration:jobAction;
    }
