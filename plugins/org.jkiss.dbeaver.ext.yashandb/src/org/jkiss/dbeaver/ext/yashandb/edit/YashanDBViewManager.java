...
public class YashanDBViewManager extends SQLTableManager<YashanDBView, YashanDBSchema> {
    ...
    @Override
    protected void validateObjectProperties(DBRProgressMonitor monitor, SQLObjectEditor<YashanDBView, YashanDBSchema>.ObjectChangeCommand command,
    ...
    protected void addObjectDeleteActions(DBRProgressMonitor monitor, DBCExecutionContext executionContext, List<DBEPersistAction> actions, ObjectDeleteCommand command, Map<String, Object> options) {
        ...
        actions.add(
                new SQLDatabasePersistAction("Drop view", "DROP VIEW " + object.getFullyQualifiedName(DBPEvaluationContext.DDL))
        );
    }
    ...
    private void createOrReplaceViewQuery(DBRProgressMonitor monitor, @NotNull List<DBEPersistAction> actions, DBECommandComposite<YashanDBView, PropertyHandler> command, Map<String, Object> options, boolean isCreate) throws DBException {
        ...
        if (!(hasComment && command.getProperties().size() == 1) && CommonUtils.getOption(options, DBPScriptObject.OPTION_OBJECT_SAVE)) {
