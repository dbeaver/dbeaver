...
public class YashanDBSchema extends YashanDBGlobalObject implements DBSSchema, DBPRefreshableObject,
    ...
    // the definition of table cache which is initialized as inner class.
    ...
    // the constructor of YashanDBSchema.
        ...
        try (JDBCSession session = DBUtils.openMetaSession(monitor, this, "Load table status")) {
            ...
            try (JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT SEGMENT_NAME,SUM(bytes) TABLE_SIZE\n" +
                            "FROM " + YashanDBUtils.getSysSchemaPrefix(getDataSource()) + (hasDBA ? "DBA_SEGMENTS" : "USER_SEGMENTS") + " " +
                            "s\n" +
    ...
    private static YashanDBTableColumn getTableColumn(JDBCSession session, YashanDBTableBase parent, ResultSet dbResult, String columnName) throws DBException {

        YashanDBTableColumn tableColumn = columnName == null ? null : parent.getAttribute(session.getProgressMonitor(), columnName);
        if (tableColumn == null) {
    ...
    /**
     * the definition of inner static class TableCache.
     */
    public class TableCache extends JDBCStructLookupCache<YashanDBSchema, YashanDBTableBase, YashanDBTableColumn> {
        ...
        /**
         * return statement by searching sql.
         */
        @Override
        ...
        /**
         * fetch the table object from abResult,and return the object of YashanDBTable which depend on tableType.
         */
        @Override
        protected YashanDBTableBase fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet dbResult) throws SQLException, DBException {
        ...
        @Override
        protected JDBCStatement prepareChildrenStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull YashanDBTableBase forTable) throws SQLException {

            String colsView;
            if (!owner.getDataSource().isViewAvailable(session.getProgressMonitor(), YashanDBConstants.SCHEMA_SYS, "ALL_TAB_COLS")) {
                colsView = "TAB_COLUMNS";
            } else {
                colsView = "TAB_COLS";
            }
//            String s1="SELECT c.*,c.TABLE_NAME as OBJECT_NAME FROM ";
            String s2="SELECT distinct(c.*),c.TABLE_NAME as OBJECT_NAME FROM ";
//            String subSql=forTable.getAdditionalInfo().toString().contains("YashanDBMaterializedView")?s2:s1;
            StringBuilder sql = new StringBuilder(500);
            sql
                    .append(s2).append(YashanDBUtils.isAdminPriv(getDataSource(), colsView))
                    .append(" c\n" + "WHERE c.OWNER=?");
        ...
        @Override
        protected YashanDBTableColumn fetchChild(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull YashanDBTableBase table, @NotNull JDBCResultSet dbResult) throws SQLException, DBException {
     ...
     */
    class ConstraintCache extends JDBCCompositeCache<YashanDBSchema, YashanDBTableBase, YashanDBTableConstraint, YashanDBTableConstraintColumn> {
        ConstraintCache() {
        ...
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(JDBCSession session, YashanDBSchema owner, YashanDBTableBase forTable)
                throws SQLException {

            boolean useSimpleConnection =
                    CommonUtils.toBoolean(session.getDataSource().getContainer().getConnectionConfiguration().getProviderProperty(YashanDBConstants.PROP_METADATA_USE_SIMPLE_CONSTRAINTS));

            StringBuilder sql = new StringBuilder(500);
            JDBCPreparedStatement dbStat;

            sql
                    .append("SELECT ").append(YashanDBUtils.getSysCatalogHint(owner.getDataSource())).append("\n" +
        ...
        @Nullable
        @Override
        protected YashanDBTableConstraint fetchObject(JDBCSession session, YashanDBSchema owner, YashanDBTableBase parent, String indexName,
        ...
        @Nullable
        @Override
        protected YashanDBTableConstraintColumn[] fetchObjectRow(
                JDBCSession session,
                YashanDBTableBase parent, YashanDBTableConstraint object, JDBCResultSet dbResult)
                throws SQLException, DBException {
            ...
            if (JDBCUtils.safeGetString(dbResult, "COLUMN_NAMES_NUMS") != null) {
                ...
                YashanDBTableConstraintColumn[] result = new YashanDBTableConstraintColumn[positions.size()];

                for (int idx = 0; idx < positions.size(); idx++) {

                    final YashanDBTableColumn column = getTableColumn(session, parent, dbResult, positions.get(idx).getColumn());

                    if (column == null) {
                        continue;
                    }

                    result[idx] = new YashanDBTableConstraintColumn(
                            object,
                            column,
                ...
                final YashanDBTableColumn tableColumn = getTableColumn(session, parent, dbResult, JDBCUtils.safeGetStringTrimmed(dbResult,
                        "COLUMN_NAME"));
                return tableColumn == null ? null : new YashanDBTableConstraintColumn[]{new YashanDBTableConstraintColumn(
                        object,
                        tableColumn,
        ...
        @Override
        protected void cacheChildren(DBRProgressMonitor monitor, YashanDBTableConstraint constraint, List<YashanDBTableConstraintColumn> rows) {
     ...
     */
    static class DataTypeCache extends JDBCObjectCache<YashanDBSchema, YashanDBDataType> {
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema owner) throws SQLException {
            JDBCPreparedStatement dbStat = session.prepareStatement("SELECT " + YashanDBUtils.getSysCatalogHint(owner.getDataSource()) + " * " +
                    "FROM " + YashanDBUtils.isAdminPriv(owner.getDataSource(), "TYPES") +
                    " " +
        ...
        @Override
        protected YashanDBDataType fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet resultSet) throws SQLException {
    ...
    class ForeignKeyCache extends JDBCCompositeCache<YashanDBSchema, YashanDBTable, YashanDBTableForeignKey, YashanDBTableForeignKeyColumn> {
        ...
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(JDBCSession session, YashanDBSchema owner, YashanDBTable forTable)
                throws SQLException {
            boolean useSimpleConnection =
                    CommonUtils.toBoolean(session.getDataSource().getContainer().getConnectionConfiguration().getProviderProperty(YashanDBConstants.PROP_METADATA_USE_SIMPLE_CONSTRAINTS));
            ...
            sql.append("SELECT " + YashanDBUtils.getSysCatalogHint(owner.getDataSource()) + " \r\n" +
                    "c.TABLE_NAME, c.CONSTRAINT_NAME,c.CONSTRAINT_TYPE,c.STATUS,c.R_OWNER,c.R_CONSTRAINT_NAME,rc.TABLE_NAME as " +
        ...
        @Nullable
        @Override
        protected YashanDBTableForeignKey fetchObject(JDBCSession session, YashanDBSchema owner, YashanDBTable parent, String indexName,
                                                      JDBCResultSet dbResult)
                throws SQLException, DBException {
        ...
        @Nullable
        @Override
        protected YashanDBTableForeignKeyColumn[] fetchObjectRow(
                JDBCSession session,
                YashanDBTable parent, YashanDBTableForeignKey object, JDBCResultSet dbResult)
                throws SQLException, DBException {

            //resultset has field COLUMN_NAMES_NUMS - special query was used
            YashanDBTableColumn column = getTableColumn(session, parent, dbResult, JDBCUtils.safeGetStringTrimmed(dbResult,
                    "COLUMN_NAME"));
            ...
            return new YashanDBTableForeignKeyColumn[]{new YashanDBTableForeignKeyColumn(
                    object,
                    column,
                    1 + JDBCUtils.safeGetInt(dbResult, "POSITION"))};
        }

        @Override
        @SuppressWarnings("unchecked")
        protected void cacheChildren(DBRProgressMonitor monitor, YashanDBTableForeignKey foreignKey, List<YashanDBTableForeignKeyColumn> rows) {
     ...
     */
    class IndexCache extends JDBCCompositeCache<YashanDBSchema, YashanDBTablePhysical, YashanDBTableIndex, YashanDBTableIndexColumn> {
        IndexCache() {
        ...
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(JDBCSession session, YashanDBSchema owner, YashanDBTablePhysical forTable)
                throws SQLException {
            StringBuilder sql = new StringBuilder();
            sql.append("SELECT ").append(YashanDBUtils.getSysCatalogHint(owner.getDataSource())).append(" " +
                    "i.OWNER,i.INDEX_NAME,i.INDEX_TYPE,i.TABLE_OWNER,i.TABLE_NAME,i.UNIQUENESS,i.TABLESPACE_NAME,i.STATUS,i.NUM_ROWS,i" +
                    ".SAMPLE_SIZE,\n" +
                    "ic.COLUMN_NAME,ic.COLUMN_POSITION,ic.COLUMN_LENGTH,ic.DESCEND\n" +
                    "FROM " + YashanDBUtils.isAdminPriv(getDataSource(), "INDEXES") + " i\n" +
                    "JOIN " + YashanDBUtils.isAdminPriv(getDataSource(), "IND_COLUMNS") + " ic " +
        ...
        @Nullable
        @Override
        protected YashanDBTableIndex fetchObject(JDBCSession session, YashanDBSchema owner, YashanDBTablePhysical parent, String indexName,
                                                 JDBCResultSet dbResult)
                throws SQLException, DBException {
        ...
        @Nullable
        @Override
        protected YashanDBTableIndexColumn[] fetchObjectRow(
                JDBCSession session,
                YashanDBTablePhysical parent, YashanDBTableIndex object, JDBCResultSet dbResult)
                throws SQLException, DBException {
            ...
            boolean isAscending = "N".equals(JDBCUtils.safeGetStringTrimmed(dbResult, "DESCEND"));
            ...
            return new YashanDBTableIndexColumn[]{new YashanDBTableIndexColumn(
                    object,
                    tableColumn,
                    ordinalPosition,
                    isAscending)};
        ...
        @Override
        protected void cacheChildren(DBRProgressMonitor monitor, YashanDBTableIndex index, List<YashanDBTableIndexColumn> rows) {
            index.setColumns(rows);
        }
    }

    /**
     * synonym cache implementation
     */
    static class SynonymCache extends JDBCObjectLookupCache<YashanDBSchema, YashanDBSynonym> {
        @NotNull
        @Override
        public JDBCStatement prepareLookupStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, YashanDBSynonym object,
                                                    String objectName) throws SQLException {
            String synonymTypeFilter =
                    (session.getDataSource().getContainer().getPreferenceStore().getBoolean(YashanDBConstants.PREF_DBMS_READ_ALL_SYNONYMS) ?
                            "" :
            ...
            StringBuilder sql = new StringBuilder();
            sql.append("SELECT OWNER, SYNONYM_NAME, MAX(TABLE_OWNER) as TABLE_OWNER, MAX(TABLE_NAME) as TABLE_NAME, MAX(DB_LINK) as " +
                    "DB_LINK, MAX(OBJECT_TYPE) as OBJECT_TYPE FROM (\n")
            ...
            sql
                    .append("\nUNION ALL\n")
                    .append("SELECT S.*,O.OBJECT_TYPE FROM ").append(YashanDBUtils.isAdminPriv(owner.getDataSource(), "SYNONYMS")).append(" S, ")
                    .append(YashanDBUtils.isAdminPriv(owner.getDataSource(), "OBJECTS")).append(" " +
                    "O\n")
        ...
        @Override
        protected YashanDBSynonym fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
     ...
     */
    static class TriggerCache extends JDBCObjectCache<YashanDBSchema, YashanDBSchemaTrigger> {

        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema schema) throws SQLException {
            JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT *\n" +
                            "FROM " + YashanDBUtils.isAdminPriv(schema.getDataSource(), "TRIGGERS") + " WHERE OWNER=?\n" +
        ...
        @Override
        protected YashanDBSchemaTrigger fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema yashanDBSchema, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
            return new YashanDBSchemaTrigger(yashanDBSchema, resultSet);
        }
    }
    ...
    class TableTriggerCache extends JDBCCompositeCache<YashanDBSchema, YashanDBTableBase, YashanDBTableTrigger, YashanDBTriggerColumn> {
        ...
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(JDBCSession session, YashanDBSchema schema, YashanDBTableBase table) throws SQLException {
            final JDBCPreparedStatement dbStmt = session.prepareStatement(

                    "SELECT t.*, c.*, c.COLUMN_NAME AS TRIGGER_COLUMN_NAME" +
                            "\nFROM " +
        ...
        @Nullable
        @Override
        protected YashanDBTableTrigger fetchObject(JDBCSession session, YashanDBSchema schema, YashanDBTableBase table, String childName, JDBCResultSet resultSet) throws SQLException, DBException {
            return new YashanDBTableTrigger(table, resultSet);
        }

        @Nullable
        @Override
        protected YashanDBTriggerColumn[] fetchObjectRow(JDBCSession session, YashanDBTableBase table, YashanDBTableTrigger trigger, JDBCResultSet resultSet) throws DBException {
        ...
        @Override
        protected void cacheChildren(DBRProgressMonitor monitor, YashanDBTableTrigger trigger, List<YashanDBTriggerColumn> columns) {
     ...
     */
    static class SequenceCache extends JDBCObjectCache<YashanDBSchema, YashanDBSequence> {
        @NotNull
        @Override
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema owner) throws SQLException {
            final JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT " + YashanDBUtils.getSysCatalogHint(owner.getDataSource()) + " * FROM " +
                            YashanDBUtils.isAdminPriv(owner.getDataSource(), "SEQUENCES") +
        ...
        @Override
        protected YashanDBSequence fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet resultSet) throws SQLException, DBException {
     ...
     */
    static class ProceduresCache extends JDBCObjectLookupCache<YashanDBSchema, YashanDBProcedureStandalone> {
    ...
    static class PackageCache extends JDBCObjectCache<YashanDBSchema, YashanDBPackage> {
        ...
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema owner)
                throws SQLException {
            JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT " + YashanDBUtils.getSysCatalogHint(owner.getDataSource()) + " OBJECT_NAME, STATUS FROM " +
        ...
        @Override
        protected YashanDBPackage fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet dbResult)
                throws SQLException, DBException {
    ...
    static class JobCache extends JDBCObjectCache<YashanDBSchema, YashanDBJob> {
        ...
        @Override
        protected YashanDBJob fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet dbResult) throws SQLException, DBException {
    ...
    static class SchedulerJobCache extends JDBCObjectCache<YashanDBSchema, YashanDBSchedulerJob> {
        ...
        protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session, @NotNull YashanDBSchema owner)
                throws SQLException {
            JDBCPreparedStatement dbStat = session.prepareStatement(
                    "SELECT * FROM " + YashanDBUtils.getAdminAllViewPrefix(session.getProgressMonitor(), owner.getDataSource(),
                            "SCHEDULER_JOBS") + " WHERE OWNER=? ORDER BY JOB_NAME");
            dbStat.setString(1, owner.getName());
            return dbStat;
        }

        @Override
        protected YashanDBSchedulerJob fetchObject(@NotNull JDBCSession session, @NotNull YashanDBSchema owner, @NotNull JDBCResultSet dbResult)
                throws SQLException, DBException {
    ...
    /**
     * the Association label is related to items label's property in Plugin.xml(which property="schemas" is related getTables function).
     */
    @Association
    public Collection<YashanDBTable> getTables(DBRProgressMonitor monitor)
            throws DBException {
        return tableCache.getTypedObjects(monitor, this, YashanDBTable.class);
    }

    public YashanDBTable getTable(DBRProgressMonitor monitor, String name)
            throws DBException {
        return tableCache.getObject(monitor, this, name, YashanDBTable.class);
    }

    @Association
    public Collection<YashanDBView> getViews(DBRProgressMonitor monitor)
            throws DBException {
        return tableCache.getTypedObjects(monitor, this, YashanDBView.class);
    }

    public YashanDBView getView(DBRProgressMonitor monitor, String name)
            throws DBException {
        return tableCache.getObject(monitor, this, name, YashanDBView.class);
    }

    @Association
    public Collection<YashanDBMaterializedView> getMaterializedViews(DBRProgressMonitor monitor)
            throws DBException {
        return tableCache.getTypedObjects(monitor, this, YashanDBMaterializedView.class);
    }

    @Association
    public YashanDBMaterializedView getMaterializedView(DBRProgressMonitor monitor, String name)
            throws DBException {
        return tableCache.getObject(monitor, this, name, YashanDBMaterializedView.class);
    }

    @Association
    public Collection<YashanDBSynonym> getSynonyms(DBRProgressMonitor monitor)
            throws DBException {
        return synonymCache.getAllObjects(monitor, this);
    }

    @Association
    public YashanDBSynonym getSynonym(DBRProgressMonitor monitor, String name)
            throws DBException {
        return synonymCache.getObject(monitor, this, name);
    }

    @Association
    public Collection<YashanDBSchemaTrigger> getTriggers(DBRProgressMonitor monitor)
            throws DBException {
        return triggerCache.getAllObjects(monitor, this);
    }

    @Association
    public Collection<YashanDBTableTrigger> getTableTriggers(DBRProgressMonitor monitor)
            throws DBException {
        // clear tableTriggerCache for executing the logic of first initializing tableTriggerCache.
        // cause objectCache/tableTriggerCache will save former triggers info which makes return objectList/getAllObjects() is null by executing objectCache logic.
        tableTriggerCache.clearCache();
        return tableTriggerCache.getAllObjects(monitor, this);
    }

    @Association
    public Collection<YashanDBDataType> getDataTypes(DBRProgressMonitor monitor)
            throws DBException {
        return dataTypeCache.getAllObjects(monitor, this);
    }

    public YashanDBDataType getDataType(DBRProgressMonitor monitor, String name)
            throws DBException {
        YashanDBDataType type = isPublic() ? getTypeBySynonym(monitor, name) : dataTypeCache.getObject(monitor, this, name);
        if (type == null) {
            if (!isPublic()) {
    ...
    @Association
    public Collection<YashanDBSequence> getSequences(DBRProgressMonitor monitor)
            throws DBException {
        return sequenceCache.getAllObjects(monitor, this);
    }

    @Association
    public Collection<YashanDBPackage> getPackages(DBRProgressMonitor monitor)
            throws DBException {
    ...
    @Association
    public Collection<YashanDBProcedureStandalone> getProcedures(DBRProgressMonitor monitor)
            throws DBException {
    ...
    @Association
    public Collection<YashanDBSchedulerJob> getSchedulerJobs(DBRProgressMonitor monitor)
            throws DBException {
    ...
    @Nullable
    private YashanDBDataType getTypeBySynonym(DBRProgressMonitor monitor, String name) throws DBException {
        final YashanDBSynonym synonym = synonymCache.getObject(monitor, this, name);
        if (synonym != null && (synonym.getObjectType() == YashanDBObjectType.TYPE || synonym.getObjectType() == YashanDBObjectType.TYPE_BODY)) {
            Object object = synonym.getObject(monitor);
            if (object instanceof YashanDBDataType) {
                return (YashanDBDataType) object;
            }
        }
        return null;
    }
    ...
    public DBSObject getChild(DBRProgressMonitor monitor, String childName) throws DBException {
        ...
        if (SYNONYMS_AS_CHILDREN) {
            YashanDBSynonym synonym = synonymCache.getObject(monitor, this, childName);
            if (synonym != null) {
                return synonym;
            }
        }
