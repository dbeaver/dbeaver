...
public class SnowflakeDataSource extends GenericDataSource {
    ...
    public GenericCatalog getCatalog(@NotNull String name) {
        return DBUtils.findObject(getCatalogs(), name, SnowflakeSQLDialect.isCaseInsensitiveIdentifier(name));
    }

    @Nullable
    @Override
    public GenericSchema getSchema(String name) {
        return DBUtils.findObject(getSchemas(), name, SnowflakeSQLDialect.isCaseInsensitiveIdentifier(name));
    }
}
