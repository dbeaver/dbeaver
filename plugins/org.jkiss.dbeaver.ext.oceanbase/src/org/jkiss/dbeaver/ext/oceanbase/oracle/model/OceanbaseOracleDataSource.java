...
public class OceanbaseOracleDataSource extends OracleDataSource {
	...
	static class OceanbaseSchemaCache extends JDBCObjectCache<OceanbaseOracleDataSource, OceanbaseOracleSchema> {
		...
		protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session,
				@NotNull OceanbaseOracleDataSource owner) throws SQLException {
			StringBuilder schemasQuery = new StringBuilder();
			// PROP_CHECK_SCHEMA_CONTENT set to true when option "Hide empty schemas" is set
			boolean showAllSchemas = !CommonUtils.toBoolean(owner.getContainer().getConnectionConfiguration()
					.getProviderProperty(OracleConstants.PROP_CHECK_SCHEMA_CONTENT));
			schemasQuery.append("SELECT U.* FROM ")
					.append(OracleUtils.getAdminAllViewPrefix(session.getProgressMonitor(), owner, "USERS"))
					.append(" U\n");
			...
			if (showAllSchemas) {
				...
				schemasQuery.append("U.USERNAME IN (SELECT DISTINCT OWNER FROM ")
						.append(OracleUtils.getAdminAllViewPrefix(session.getProgressMonitor(), owner, "OBJECTS"))
						.append(")");
			}

			DBSObjectFilter schemaFilters = owner.getContainer().getObjectFilter(OceanbaseOracleSchema.class, null,
					false);
		...
		@Override
		protected OceanbaseOracleSchema fetchObject(@NotNull JDBCSession session,
				@NotNull OceanbaseOracleDataSource owner, @NotNull JDBCResultSet resultSet)
				throws SQLException, DBException {
			return new OceanbaseOracleSchema(owner, resultSet);
		}

		@Override
		protected void invalidateObjects(DBRProgressMonitor monitor, OceanbaseOracleDataSource owner,
				Iterator<OceanbaseOracleSchema> objectIter) {
			setListOrderComparator(DBUtils.<OceanbaseOracleSchema>nameComparator());
		}
	}
	...
	static class OceanbaseUserCache extends JDBCObjectCache<OceanbaseOracleDataSource, OceanbaseOracleUser> {
		...
		protected JDBCStatement prepareObjectsStatement(@NotNull JDBCSession session,
			...
			return session.prepareStatement(
					"SELECT * FROM " + OracleUtils.getAdminAllViewPrefix(session.getProgressMonitor(), owner, "USERS")
							+ " ORDER BY USERNAME");
		}

		@Override
		protected OceanbaseOracleUser fetchObject(@NotNull JDBCSession session,
				@NotNull OceanbaseOracleDataSource owner, @NotNull JDBCResultSet resultSet)
				throws SQLException, DBException {
