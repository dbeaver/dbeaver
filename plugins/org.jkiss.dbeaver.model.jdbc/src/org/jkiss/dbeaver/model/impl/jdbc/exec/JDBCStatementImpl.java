...
public class JDBCStatementImpl<STATEMENT extends Statement> extends AbstractStatement<JDBCSession> implements JDBCStatement {
    ...
    private String addQueryIdentificationComment(@NotNull String sql) {
        DBCExecutionPurpose purpose = getSession().getPurpose();
        if (purpose.isUser()) {
            return sql;
        }
        var prefStore = getConnection().getDataSource().getContainer().getPreferenceStore();
        if (!prefStore.getBoolean(ModelPreferences.META_ADD_AUTO_QUERY_IDENTIFICATION_COMMENT)) {
            return sql;
        }
        return SQLUtils.generateCommentLine(connection.getDataSource(), GeneralUtils.getProductName()) + "\n" + sql;
    }
    ...
    public boolean execute(@NotNull String sql) throws SQLException {
        sql = addQueryIdentificationComment(sql);
        setQueryString(sql);
        this.beforeExecute();
        try {
