...
public class SearchWithBingCommand extends AbstractHandler implements IElementUpdater {
    private static final String TITLE = "Search selection with Bing";
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        String bingLink = textSelection.getText().trim();
        bingLink = URLEncoder.encode(bingLink, StandardCharsets.UTF_8);
        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
            ShellUtils.launchProgram(SEARCH_WEB_ADDRESS_PREFIX + bingLink);
        } else {
    ...
    public void updateElement(UIElement element, Map parameters) {
        ...
        element.setTooltip(SQLEditorMessages.editors_sql_actions_search_selected_text_online_bing_tip);
    }
}
