...
class GridCellSpanManager {
    List<Rectangle> listOfCellSpanRectangles = new ArrayList<Rectangle>();
    ...
    {
        for (Rectangle cellSpanRectangle : listOfCellSpanRectangles) {
    ...
    {
        Rectangle rectangleToConsume;

        if (this.lastUsedCellSpanRectangle != null
            && this.lastUsedCellSpanRectangle.contains(columnIndex, rowIndex))
        {
        ...
        if (rectangleToConsume != null) {
            if (columnIndex >= rectangleToConsume.x + (rectangleToConsume.width - 1)
                && rowIndex >= (rectangleToConsume.y + rectangleToConsume.height - 1))
            {
