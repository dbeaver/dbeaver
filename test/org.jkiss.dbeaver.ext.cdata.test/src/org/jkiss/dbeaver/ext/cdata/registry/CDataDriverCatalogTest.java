...
public class CDataDriverCatalogTest extends DBeaverUnitTest {
    ...
    public void loadCatalog() {
        ...
        // the Maven artifact is catalog data, not something the code derives from the data source name
        Assertions.assertTrue(drivers.stream()
            .allMatch(driver -> driver.artifactId().endsWith("-jdbc")));
        Assertions.assertTrue(drivers.stream()
            .allMatch(driver -> driver.artifactId().equals(driver.jdbcName() + "-jdbc")));
        Assertions.assertEquals("googleads-jdbc", drivers.stream()
            .filter(driver -> driver.dataSource().equals("adwords"))
            .findFirst()
            .orElseThrow()
            .artifactId());
        Assertions.assertEquals("googleads", drivers.stream()
            .filter(driver -> driver.dataSource().equals("adwords"))
            .findFirst()
            .orElseThrow()
            .jdbcName());
