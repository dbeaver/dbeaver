...
public class ProgressStreamReaderTest extends DBeaverUnitTest {
    ...
    @Test
    public void readByteArrayWithOffsetAtEofDoesNotReportProgress() throws IOException {
        ProgressStreamReader reader = reader();
        byte[] buffer = new byte[4];

        Assertions.assertEquals(-1, reader.read(buffer, 1, 2));

        Mockito.verify(monitor, Mockito.never()).worked(Mockito.anyInt());
    }

    @Test
    public void readZeroLengthDoesNotReportProgress() throws IOException {
        ProgressStreamReader reader = reader(1, 2, 3);

        Assertions.assertEquals(0, reader.read(new byte[0]));
        Assertions.assertEquals(0, reader.read(new byte[3], 0, 0));
