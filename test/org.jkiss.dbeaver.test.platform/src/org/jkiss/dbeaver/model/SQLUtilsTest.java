...
public class SQLUtilsTest extends DBeaverUnitTest {
    ...
    public void addQueryIdentificationCommentTest() {
        ...
        DBPPreferenceStore prefStore = Mockito.mock(DBPPreferenceStore.class);
        SQLDialect dialect = Mockito.mock(SQLDialect.class);

        Mockito.lenient().when(session.getDataSource()).thenReturn(dataSource);
        Mockito.lenient().when(dataSource.getContainer()).thenReturn(container);
        Mockito.lenient().when(container.getPreferenceStore()).thenReturn(prefStore);
        Mockito.lenient().when(dataSource.getSQLDialect()).thenReturn(dialect);
        Mockito.lenient().when(dialect.getSingleLineComments()).thenReturn(new String[]{"--"});

        // disabled
        Mockito.when(prefStore.getBoolean(ModelPreferences.META_ADD_AUTO_QUERY_IDENTIFICATION_COMMENT)).thenReturn(false);
        String query = "SELECT 1";
        Assert.assertEquals(query, SQLUtils.addQueryIdentificationComment(session, query));

        //  USER
        Mockito.when(prefStore.getBoolean(ModelPreferences.META_ADD_AUTO_QUERY_IDENTIFICATION_COMMENT)).thenReturn(true);
        DBCExecutionPurpose purposeUser = DBCExecutionPurpose.USER;
        Mockito.when(session.getPurpose()).thenReturn(purposeUser);
        Assert.assertEquals(query, SQLUtils.addQueryIdentificationComment(session, query));

        // USER_SCRIPT
        DBCExecutionPurpose purposeScript = DBCExecutionPurpose.USER_SCRIPT;
        Mockito.when(session.getPurpose()).thenReturn(purposeScript);
        Assert.assertEquals(query, SQLUtils.addQueryIdentificationComment(session, query));

        Mockito.when(session.getPurpose()).thenReturn(purposeUser);
        String queryWithComment = "-- Query automatically generated by DBeaver (auto-gen)\nSELECT 1";
        Assert.assertEquals(queryWithComment, SQLUtils.addQueryIdentificationComment(session, queryWithComment));

        // META
        DBCExecutionPurpose purposeMeta = DBCExecutionPurpose.META;
        Mockito.when(session.getPurpose()).thenReturn(purposeMeta);
        String result = SQLUtils.addQueryIdentificationComment(session, query);
        Assert.assertTrue(result.contains("Query automatically generated by"));
        Assert.assertTrue(result.contains("(auto-gen)"));
