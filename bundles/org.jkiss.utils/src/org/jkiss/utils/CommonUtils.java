...
public class CommonUtils {
	...
	/**
	 * Swaps the element with its neighbor to the left in the specified list. If the
	 * element is not present in the list or it is the leftmost element in the list,
	...
	/**
	 * Swaps the element with its neighbor to the right in the specified list. If
	 * the element is not present in the list or it is the rightmost element in the
	 * list, the list remains unchanged.
	 *
	 * @param list    list
	 * @param element element
	 * @param <T>     type of the list
...
//        }
	...
	@Nullable
	public static <T extends Enum<T>> T valueOf(@NotNull Class<T> type, @Nullable String name, T defValue,
	...
	public static String getSingleLineString(String displayString) {
		return displayString.replace('\n', PARAGRAPH_CHAR).replace("\r", "").replace("\t", " ").replace((char) 0, ' ');
	}
	...
	/**
	 * Checks whether the supplied <code>ch</code> is within the range of the
	 * specified <code>radix</code> value.
	 *
	 * @param ch    character codepoint to be checked
	 * @param radix desired radix
	 * @return <code>true</code> if the character fits into the radix,
	...
	/**
	 * Checks if the {@code index} is within the bounds of the range from {@code 0}
	...
	/**
	 * Finds the object with the best matching name. Here we consider a case
	 * sensitive match better then a case insensitive one.
	 *
	...
	/**
	 * Groups values into a map of their shared key and a list of matching values
	 * using that key.
	 * <p>
	 ...
	 * @param keyExtractor a function that extracts key from value that is used to
	 *                     group values
	 * @return map of a shared key and a list of matching values
	 */
	@NotNull
	public static <K, V> Map<K, List<V>> group(@NotNull Collection<V> values,
	 ...
	 * @return {@code min} if {@code value} is less than {@code min}, {@code max} if
	 *         {@code value} is greater than {@code max}, otherwise {@code value}
	 */
	 ...
	 * with the result of applying the given replacer function to the match result
	 * of this matcher corresponding to that subsequence.
	 */
	@NotNull
	public static String replaceAll(@NotNull String input, @NotNull String regex,
	...
	/**
	 * Removes (single or multiple) starting '/' from {@code resourcePath} and
	 * replaces all '\' with '/'
	 */
	@NotNull
	...
	/**
	 * Replaces the last {@code toReplace} entry in {@code string} with
	 * {@code replacement}
	 */
	@NotNull
